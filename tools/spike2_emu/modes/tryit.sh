...
put() {   # put <src> <dest>
    cp "$1" "$2.tryit.tmp" && mv -f "$2.tryit.tmp" "$2" || { rm -f "$2.tryit.tmp"; return 1; }
}
...
case "$cmd" in
    install)
        ...
        for d in "$LIB" "$DUMP"; do
            why=$(blocker "$d") && die "cannot put the modes in the emulator: $why (an emulator run" \
                "as root made it). Hand it back with: wsl -u root chown -R $(id -un):$(id -gn) $d"
        done
    ...
    push)
        ...
        ;;
    *)
        die "usage: tryit.sh install <stage> | start [K] | stop [NAME...] | push <file> <K>"
        ;;
esac
