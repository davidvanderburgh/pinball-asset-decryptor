...
- **Jaws LE 1.01.0 is unresolved, and deliberately reported that way.** Neither locator matches, in either of its game binaries (`/jaws_le/game` and `spike_menu/game`); no ELF anywhere on that card carries more than one `0xEDB88320`; it is not Thumb (same `e_flags` and ARM entry as every other card); and its six `#N ... UPDATE SD CARD` messages are present and byte-identical. Tracing back from those messages leads nowhere — on cards where the validator *is* known, nothing that references them calls it or is called by it, so they are reached through data tables. Concluding "Jaws has no validator" would be inferring absence from our own signature failing, on a *newer* title where the check is likelier to have been strengthened than dropped. So `carries_validator` keeps keying on the messages, Jaws reports `unlocated`, and the Write warns. A warning on a card that may not need one is cheap; silence on a card that does is a hardware test that fails invisibly. `bypass_status` returns `bypassed` / `absent` / `unlocated`, and only `unlocated` warns — on the build log at `error` level and in the Write Complete dialog.
