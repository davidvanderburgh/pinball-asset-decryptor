...
directly — no mounting, no WSL — and right-click **Replace with…** swaps
any file on the card for one of your own, at any size:
...
9. **Emulate tab** *(Stern Spike 2 — Windows via WSL2, Linux, or macOS in a container)* — run the
   ...
   guest out of a card image — `rootfs.sh <card.raw>`, which needs no
   root; the hardware shim and the GL renderer are compiled by the first
   run that needs them, and rebuilt whenever an update moves their
   sources on. It is a Linux program
