...
**Unmapped builds fail fast, never hang.** A build newer than the locator knows can land the master-dir malloc hook on a PC where r5 is *not* the record count; accepting that garbage disarmed the fail-fast watchdog and drove `_ensure_range`/`mem_read` over `n * 24` bytes — a field report saw "Deriving codec parameters" sit for 24+ minutes with no error (a Deadpool Pro 1.15 card). The capture now sanity-gates the count (`_accept_masterdir_malloc`, same `MAX_RECORDS` bound as the per-category gate), so the watchdog stays armed and the derive bails within ~1–2 min into the existing "could not map this firmware build" path (which saves the firmware next to the output for a locator fix).
