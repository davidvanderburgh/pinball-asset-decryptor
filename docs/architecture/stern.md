...
**The record count comes from the served malloc, not a fixed register.** Deadpool Pro 1.16 (and the 1.15 build behind a "Deriving codec parameters sits forever" field report) reuses r5 for the *aligned byte size* it passes to the record-array malloc (`add r5, r7, #0xf ; bic r5, r5, #0xf`) — every earlier build kept the raw count live in r5 at the malloc-return PC, so the old fixed `r5` read returned ~24× the real count there and chained the derive through tens of thousands of garbage records. The import stub *serves* that malloc, so it records `(ptr, size)` and `_md_record_count` derives the count from the size when the captured buffer is that ptr (`size // 24`, exact — the 16-byte alignment pad is < 24), keeping r5 only as the fallback for an unserved allocation. A count that is still insane is rejected (`_accept_masterdir_malloc`, same `MAX_RECORDS` bound as the per-category gate), which keeps the fail-fast watchdog armed so a genuinely unmapped build bails within ~1–2 min into the existing "could not map this firmware build" path (which saves the firmware next to the output for a locator fix) instead of hanging.
