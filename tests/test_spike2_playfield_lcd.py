...
def _write_still(d, name, color="green", w=240, h=180):
    ...
    img.write(os.path.join(d, name), format="png")
...
def test_mapped_id_shows_its_still_never_the_clip(tmp_path, monkeypatch):
    ...
    try:
        ...
        assert p.nm["text"] == "BATMAN logo card", p.nm["text"]
...
def test_block_command_selects_by_its_first_id(tmp_path, monkeypatch):
    ...
    try:
        ...
        assert p.nm["text"] == "BATMAN IN COLOR card"
    finally:
        root.destroy()
...
def test_brightness_drop_dissolves_only_the_screen(tmp_path, monkeypatch):
    ...
    the image, every dissolve step keeps the item VISIBLE - end state is
    a black screen in a lit set - and 255 restores the picture."""
    root = _root()
    try:
        playfield, p, block = _panel(root, str(tmp_path), monkeypatch)
        ...
        for k in range(4):                  # steps + dark end state
            _poll(p)
            assert p.cv.itemcget(p.item, "state") == "normal",                 "the set vanished at dissolve step %d" % k
        assert not p._fadeq
