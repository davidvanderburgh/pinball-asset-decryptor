...
#   NEEDED only libEGL.so.1 libGLESv2.so.2 libasound.so.2 libpthread.so.0 libc.so.6 libm.so.6 libgcc_s.so.1
#   (libpthread.so.0 since 2.8: the input scan and the frame cache are threads; the
#   rootfs ships /lib/libpthread-2.21.so)
...
for n in $needed; do
    case "$n" in
        libEGL.so.1|libGLESv2.so.2|libasound.so.2|libpthread.so.0|libc.so.6|libm.so.6|libgcc_s.so.1) ;;
