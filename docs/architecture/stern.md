...
- **What may go on the card untouched.** An oversized clip avoids that byte-budget crush by *growing* its slot through the ext4 driver instead (`_prepare_video_patches` → `core/ext4_grow`), and that copy is byte-for-byte — which means whatever the user picked in the file dialog lands on the card. The machine's decoder is an i.MX6 VPU, not a desktop player: give it a Matroska container, HEVC, a 10-bit / 4:2:2 stream, or a resolution the scene never sized a surface for, and it demuxes far enough to **play the sound over a black picture**, with nothing said at build time. So `_intact_copy_source` ([engine.py:1988](../../pinball_decryptor/plugins/stern/engine.py#L1988)) gates the verbatim copy on the file actually being a drop-in for the clip it replaces — same container brand (the extract extension records whether the card's own clip was `qt  `-branded), H.264, 8-bit 4:2:0, and the slot's own resolution and frame rate. The staged copy is the yardstick for the last two, since Replace-Video already format-matched it to that exact slot. A source failing any check falls back to that staged copy, logged with the reason; it still grows to full size, so the only thing lost is the re-encode the user's own file would have avoided. This also makes the list's **Convert** column honest: a row that says `Re-encode` now really is re-encoded.
