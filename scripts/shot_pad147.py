"""PAD-147: the boot screen (SternLogo.png on sda2) on the Replace Images tab.

    python scripts/shot_pad147.py <card.raw> <extract_dir> <out_dir> <before|after>

One shot, ``<before|after>_images_tab.png``: the Replace Images tab over
*extract_dir* (an Extract of *card.raw*) with "SternLogo" in its search box
and the first hit selected.  Before the change the extract never held the
boot screen, so the search comes back empty; after it, the picture the
machine shows while it starts up is a row with its preview.

NOTHING IS WRITTEN TO THE CARD OR THE EXTRACT: no replacement is assigned
and no build runs.  settings.json is backed up and restored, and the session
log is sandboxed.
"""
import ctypes
import json
...
stern.update(extract_input=CARD, write_original=CARD,
             extract_output=EXTRACT, write_assets=EXTRACT)
with open(SETTINGS, "w", encoding="utf-8") as f:
    json.dump(settings, f, indent=2)

from pinball_decryptor.core import session_log  # noqa: E402
session_log.LOG_DIR_OVERRIDE = tempfile.mkdtemp(prefix="pad147_log_")
...
def s_select():
    ...
    log("rows matching %r: %d" % (SEARCH, len(rows)))
    for iid in rows:
        log("  row: %s" % iid)
    if rows:
        tree.see(rows[0])
