...
Driven through the web UI's Compare service (webui/tabs/compare.py).
"""
...
def _pump(w, done, limit=10.0):
    """Let the tab's worker and the loop job it posts actually run."""
    end = time.time() + limit
...
def test_extract_both_output_is_where_compare_looks(tmp_path):
    ...
    with web_app(tmp_path, mfr="stern") as w:
        w.run(w.window.remember_browse_dir, "extract_both", str(parent))

        from pinball_decryptor.core import extract_source
        roots = w.run(_svc(w)._extract_roots, a, b)
        assert extract_source.find_extract_for(a, roots) == out_a
        assert extract_source.find_extract_for(b, roots) == out_b


def test_the_extract_tab_and_the_card_s_own_folder_count_too(tmp_path):
    ...
    with web_app(tmp_path, mfr="stern") as w:
        w.call("ui.set", "extract", "output", str(tmp_path / "runs" / "pro"))
...
def test_no_extract_anywhere_resolves_to_nothing(tmp_path):
...
def test_compare_hands_the_extracts_to_the_plugin(tmp_path, monkeypatch):
    """End to end through the real button's call: whatever the tab resolved
    ...
    with web_app(tmp_path, mfr="stern") as w:
        ...
        monkeypatch.setattr(w.window.current_mfr, "compare_images", _fake)
        assert w.call("compare.run") is True
        svc = _svc(w)
        _pump(w, lambda: svc._sections)
        assert seen == {"a": a, "b": b, "xa": out_a, "xb": out_b}
        assert svc._sections == [
            ("Sounds", [("Decoded sounds", "549 (unchanged)")])]


def test_a_listed_sound_opens_out_of_the_extract_folder(tmp_path,
                                                        monkeypatch):
    ...
    with web_app(tmp_path, mfr="stern") as w:
        ...
        w.run(svc.render, [("Sounds", [("Changed", "1:"),
        ...
        # Played straight out of the extract — no temp copy of a decoded
        # sound.  (The read half and the open half are exercised separately:
        # the tab runs the read on a worker thread.)
        out = tmp_path / "temp-copy"
        out.mkdir()
        assert w.window.current_mfr.extract_report_file(
            image, ref, str(out)) == str(wav)
        ...
        def _busy():
            svc._open_busy = True
        w.run(_busy)
        w.run(svc._open_finished, ref["name"], "B", str(wav), None)
