...
- **The validator bypass can miss, and has to say so.** `find_validation_exec` picks the function carrying several inlined CRC32-`0xEDB88320` loops ([valpatch.py:1](../../pinball_decryptor/plugins/stern/valpatch.py#L1)). That is a signature of how the firmware was *compiled*, not of what it does: a build that factors its CRC32 out of line carries no `0xEDB88320` immediate in `.text` at all and matches nothing. Measured over the 34 vendor cards on hand it finds the routine on 33 and comes up empty on **Jaws LE 1.01.0**. Whether a title *has* a validator is therefore answered separately, by `carries_validator` — the six numbered `#N ... UPDATE SD CARD` messages the validator puts on the LCD, byte-identical on all 34 cards including Jaws. Without that split, "no validator here" and "its validator is armed and we couldn't reach it" both read as a no-op, and the Write ships a card the machine rejects with GAME VALIDATION ERROR while reporting success. `bypass_status` returns `bypassed` / `absent` / `unlocated`, and only `unlocated` warns — on the build log at `error` level and in the Write Complete dialog.
