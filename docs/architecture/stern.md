...
**Some sounds read the body in OVERLAPPING blocks and cannot be re-encoded at all.** The model above assumes output sample `i` reads one body word (`i + delta`). A subset of sounds instead has block `b` read words `200*b + 2*i`, so only even words are read and **consecutive blocks share them**: measured exactly on Deadpool Pro 1.16 idx7146 — all 14 blocks fit `200*b + 2*i` bit-exact, 1408 of 2908 words read (0 odd), and **1300 of those feed two output samples each** under different keystreams. One 16-bit word cannot satisfy two independent targets, so per-sample inversion cannot round-trip these sounds — and a "stride-aware" encode is not a fix either: the slot is exactly `2*length` bytes (verified across the whole catalog, `gap/2len` median 1.00), so writing two words per sample would overrun the neighbouring sound. This is **not build-specific and not new**: sampling long-shipped titles found the same shape on Godzilla 1.13 and Star Wars 1.30. Write already handles it correctly — `_recovery_valid` re-encodes the sound's own audio first and skips any sound that doesn't reproduce bit-exact ("re-encode isn't bit-exact for this sound's codec — skipped, left unchanged"), so these are left alone rather than written as noise. Decode/extract is unaffected: the firmware does the reading, so these sounds extract normally.
