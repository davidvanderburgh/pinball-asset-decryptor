...
- **To WRITE a mode, use the SDK: `modes/sdk/MODE_SDK.md` (item 134).** It puts every
  call below behind `pad_mode.h`, with no addresses in it; the addresses, globals and
  offsets are a per-build PORT file (`sdk/ports/`), checked word by word at boot. This
  file stays the record of how each call was found.
...
### Several modes on one card (item 133, emulator-proven 2026-09-16)

| Slot | File (first found wins) | Start trigger file |
|---|---|---|
| 0 | `/usr/local/padmode/mode.cfg`, then `/dump/mode.cfg` | `/dump/mode.start` |
| 1-7 | `/usr/local/padmode/modeK.cfg`, then `/dump/modeK.cfg` | `/dump/modeK.start` |

`/dump/mode.stop` ends whichever mode is running; `/dump/mode.clip` is unchanged. Each slot
reloads on its own, keeps its own trigger counts and its own screen (EVERY slot's screen is
found and hidden, since each is authored visible). **One of ours runs at a time** - one
video surface, one set of borrowed message slots - so a start while another runs logs
`<name> not started (<why>): <other> is running`. A slot whose file disappears is disarmed
(`mode file gone: slot K ("NAME") is not armed`); a run under way finishes on the config
it started with. The running mode scores a shot before any other mode counts it as a
trigger. In code, every function that reads a mode takes the slot as `M` and `cfg` is
`M->c` - a parameter, never a global, because the shot dispatch and the tick are hooks on
game threads. The sound one-shot carries its key pointer across to the lookup thread.

**Proven (one game, stock scenes):** ALPHA in `mode.cfg` and BETA in `mode1.cfg` loaded
together; BETA started from its OWN trigger (two ramp shots) and scored 7M; ALPHA's start was
refused while BETA ran; BETA's file rewritten mid-run reloaded and the next shot paid 18M
(2 x 9M); after BETA ended ALPHA started from slot 0 and scored its own shot; deleting
`mode1.cfg` disarmed BETA and its start file then did nothing. 0 segv. Not yet exercised: a
TRIGGER (rather than a start file) completing while another mode runs - the same refusal
path.
...
**PASS 4 SETTLES IT: retargeting the sid -> record node is INSUFFICIENT.** The earlier
negatives each had an escape hatch - one trigger, one moment, "maybe it was vetoed
that once". This one removes it. **Sixteen mono sids** were pointed at our appended
record (every sid the mode's own callouts reach: `request_nth 1287`'s ten, 1291's
five, 1295's one), all patched and read back clean, and re-verified at fire time. Two
mode runs produced **14 separate triggers** - and the clip appears in none of them,
correlation **0.027**. Four runs now: 0.026 / 0.033 / 0.032 / 0.027.

The net had to be built from the mode's own callouts because of a census worth
keeping: **every request that repeats during ordinary play is STEREO** - 570 x25,
415 x15, 443/436/420 x10, 437 x9, all chan 2 - so there is no frequently-fired mono
single-sid request to borrow, and our record is chan 1.

So the node fields are what they were measured to be (`+24` body_off, `+32`
length x 2 x chan, `+36` block bytes - 2581/2581 each), and they are **not what
decides which bytes play**. Whatever the worker reads the body from, this is not it.

**It IS a multimap, and that still does not explain the negative - checked, not
assumed.** `0x2a2f6c`..`0x2a3008` does lower/upper-bound scanning and calls
`_Rb_tree_increment` (`0x2a2ff8`), the shape of an `equal_range`, and a live walk
confirms duplicates exist: 2581 nodes carry only **2558 distinct sids**, with 19 sids
holding two or three nodes each (23, 2491, 2495, 2525 x3, 2529 x3, ...). So a walker
that stops at the first match CAN patch the wrong node - but **every one of the
sixteen sids patched in pass 4 has exactly one node** (854, 1233, 1998, 1881, 834 all
verified singular). The duplicates are real and irrelevant here.

### THE DESCRIPTOR KEY MASK IS PER-BUILD, and the engine has Led Zeppelin's

**`engine._DESC_KEY2_MASK = 0xE0001FFF` is Led Zeppelin 1.22's constant. Godzilla Pro
1.15 keeps a different bitfield: `0xFC0003FF`** - bits 31..26 and 9..0. Measured, not
inferred: for every sid the runtime tree knows, line the descriptor's op11 payload up
against the record's real container key and

```
findkey.w1 = payload.w1
findkey.w2 = payload.w2 & 0xFC0003FF        2557 exact, 0 wrong
```

The 23 exceptions all have a different `w1` and are exactly the duplicate-sid nodes
(23, 2491, 2495, 2498, 2517, 2519 ...) - an artefact of a first-match tree walk on a
multimap, not of the rule.

**What the wrong mask did.** It manufactured keys that matched almost nothing, so only
**44 of 2535** records looked descriptor-named and `_plan_descriptor_repoint` refused
with "no descriptor in the game's play tables names this sound's record". With the
right mask it is **2534 of 2535**, and the re-point plans, writes and **passes the
engine's own resolver verification**. So the descriptor route was never blocked; the
constant was wrong.

**This is a product bug, not just a rig one:** the same constant drives the shipped
grow path, so a grown Godzilla bank built through `engine.write_image` would hit the
same refusal (or worse, re-point against a coincidental match). Worth its own queue
item - the mask needs locating per build the way the other cipher sites already are.

### THE RE-POINT WORKS. The game binds a sid to a record the card never shipped.

With the mask corrected, `image.g1560.bin` was built with one descriptor re-pointed at
our appended record, and a guest booted on it. **In a guest nothing patched at
runtime**, the live tree reads:

```
sid 1998   +24 1649655186 (our appended body)   +32 353200   +36 400
sid 854    +24 1599642802 (stock)               sid 1233  +24 1254864682 (stock)
sid 1881   +24 1610354694 (stock)               sid 834   +24 1258604004 (stock)
```

The game's own boot-time band build resolved the re-pointed descriptor, looked the key
up in the container, and bound the node to a record the card never shipped. Exactly
one sid moved. **Resolution is solved.**

**And it still does not play**: two `callout 1295` firings, correlation 0.035.

### THE GAME PLAYS OUR RECORD. Proven by LENGTH, which no decode can fake.

Correlation had been the only instrument, and a control exposed it: fire the stock
request 1295 five times on the STOCK bank and score against the stock record's own
decoded WAV, and it scores **0.0301** - on a record that demonstrably played. So a low
correlation has never been evidence that a sound did not play.

Length cannot lie that way. The guest writes PCM only while playing, so five firings
of a 4.00 s record cannot look like five firings of a 1.89 s one. Same trigger
(`/dump/padmode.sound`), same five firings, same eight-second spacing, two banks:

| | PCM captured | per firing |
|---|---|---|
| STOCK bank (record 1560, 1.89 s) | 9.083 s | **1.817 s** |
| RE-POINTED bank (our record, 4.00 s) | 18.792 s | **3.758 s** |

Measured ratio **2.069**, against **2.119** predicted if our record plays and **1.000**
if the stock one does. 107% more audio per firing against 112% predicted. The ~4%
shortfall appears on BOTH banks (control 1.817 s vs 1.888 s native), so it is a fixed
trim - the lead-out block, or the last period - not a discrepancy between the cases.

**So the chain is complete and it is the game's own:** a descriptor re-pointed on disk
-> the boot-time band build resolves it -> the container hands back our appended record
-> the sid tree binds to it -> `sound_request_play` reads 4 seconds of a body the card
never shipped, five times out of five, `returned 2,3,4,5,6`. Item 130's mechanism
works.

### AND IT SOUNDS LIKE THE CLIP. corr 0.9921. The scorer was resampling.

The rate suspicion was right, and it was mine, not the game's. `score130.py` resampled
the reference 44100 -> 48000 to match the capture. **The game emits those samples at
48 kHz natively.** Take the reference AS 48 kHz and:

| | resampled 44.1->48 | taken as 48 kHz |
|---|---|---|
| CONTROL, stock record 1560 | 0.0301 | **0.9925** at 0.00 s |
| TEST, our appended record | 0.0120 | **0.9921** at 0.00 s |

A sweep over 32/36/40/44.1/46/48/50 kHz puts every other assumption at 0.01-0.12 and
48 kHz at 0.99 on both. So the record on the card is 44.1 kHz by its own header and the
engine plays it out at 48 kHz - a pitch shift of 8.8%, which is exactly the kind of
thing a correlation cannot survive and a length test never noticed.

**ITEM 130 IS MET: a mode plays a sound that exists nowhere in the stock game**, and it
is the game's own chain end to end - descriptor re-pointed on disk, boot-time band
build resolves it, container hands back the appended record, the sid tree binds to it,
`sound_request_play` reads 4.00 s of a body the card never shipped, five times out of
five, at **corr 0.9921** against the clip we encoded. Zero stock records moved.

**THE LESSON, and it cost most of a night:** every one of the earlier negatives was
this one wrong constant in MY instrument. The scorer had been "validated" by splicing
the clip into a capture - but that splice went in at the capture's own rate, so it
tested everything except the resample. A positive control has to travel the WHOLE path
the real measurement travels; one that shares the fault it is meant to catch proves
nothing. The stock-record control is the one that broke it open, because it failed on
a sound that had demonstrably played.

### The scorer is VALIDATED (and this should have happened six runs earlier)

Every negative here rests on one correlation tool that had never once returned a high
number. Splicing the reference clip into the real capture at a known offset:

| | correlation |
|---|---|
| clip spliced in at full level | **0.903** at the right offset |
| clip spliced in at -12 dB | **0.599** at the right offset |
| the unmodified capture | 0.035 |

So the instrument discriminates, and the negatives stand. The lesson is the same one
this file keeps recording: a measurement with no positive control is not a measurement.

**Where that leaves item 130.** The game now reads our body and emits nothing
recognisable. Our appended record is **scale 13**; the slot it replaces is **scale 1**.
The Python emulator round-trips it at corr 1.00000, but using the codec entry
`warm_slots_for_grown` seeded from a `(13,1)` stock sound - while the live decode uses
whatever the chain gives at the appended record's position. Item 104 wrote the warning
down: *"its scale is chosen by the chain, not by the sound"*.

**Next test, and it is cheap:** grow a bank whose appended record comes out at the SAME
scale as the slot it replaces, re-point, fire. If a scale-matched record plays, that
names the cause exactly. `error_log` (`0x457e00`, codes 100/101/102) remains the
fallback instrument. Still unexplained: 39 gaps in the sid space, 38 records named by
no sid.

Four passes have each turned on a name or an arithmetic reading that looked obvious
and was wrong. Everything that held was predicted first and then read back off a live
guest.
...
## A screen of our OWN: emulator-proven in a game (item 131, 2026-09-16)

Borrowing message ids (above) makes tesla's award read our words while the mode runs.
Item 131 removes the borrowing: the mode's screen is a NODE AUTHORED INTO A SCENE FILE,
with our art and our words, and the mode drives it by name through the game's own calls.
Not hardware-proven.

**Build it:** `modes/mode_screen.py <stock scene.radium> <out> --name KaijuRush_Screen
--title "KAIJU RUSH" --words "1,000,000 A SHOT"` (the writer is
`pinball_decryptor/plugins/stern/scene_write.py`). It refuses any scene it has not
measured: type ids, symbol keys and font ids differ per file.
**Drive it:** `modes/kaiju_rush_own_screen.mode` - `screen_scene`, `screen_node`,
`screen_text`, and no `title_msg`/`total_msg`.

| Step | Call | Notes |
|---|---|---|
| a scene by id | `[0x7db220]` resource manager, `0x443de4(mgr, &std::string id, 0)`, `__dynamic_cast` PLT `0x16308` (`0x6209c4` -> `0x673b94`), scene at `+0x10` | `0x3bac44` does the same but logs error 0x101 when no event is current - every tick |
| the attract scene | singleton `0x79ac34`, scene at `+0x10` | NOT drawn during a game |
| a Sprite/Bitmap node | `0x578a2c(&shared_ptr, scene, &std::string "A.B")` | a TYPED find: generic `0x577a4c` then a cast |
| a Text node | `0x5784c8(&shared_ptr, scene, &std::string "A.B")` | `0x578a2c` returns null for a Text; the HUD uses this one |
| show / hide | node `vfn[0xa8](node, bool)` | drawn = keyframes AND this flag: code can hide, never reveal |
| words | `0x55c1f4(text node, &std::string)` | what the HUD writes its own scores with |

Strings are the COW ABI (`std::string` is one pointer, built with PLT `0x16908`).
`hook.h` wraps all of it: `gz_scene`, `gz_find_node`, `gz_find_text`, `gz_node_visible`,
`gz_node_text`. The probe exposes it as `padmode.node "<0|1> [<scene id>:]<path>"`.

**Which scene.** In a game the glass is the HUD scenes, found with `scenelog.so`'s name
log: `9d578751` (top panel, player scores), `32e6ae28` (the mode slide-outs), `43b52a63`
(energy meter, high score). The screen lives in `32e6ae28`. The attract scene `394c4a03`
draws our node in attract and never during play.

**Proven, in a game on the Tokyo map:** hidden while idle; at `mode.start` our art and
"1,000,000 A SHOT"; "+1,000,000" and "+2,000,000" on the two ramp shots over the game's
own ramp videos; "TOTAL 3,000,000" at the end; hidden `restore_after` seconds later.
Tesla strike's own screen, started beside it, is frame-for-frame what the stock scene
shows. Where the panel lands moves with the HUD layer (right of centre over the map, left
over a full-screen video).

**The one hazard:** the node is authored VISIBLE and it is `mode.so` that hides it, so a
scene built this way must never be installed without the mode.

## Reading the scene loader: `modes/scenelog.so` (item 131)

The format was decoded from what the GAME READS, not inferred: `scenelog.so` hooks cereal's
`PortableBinaryInputArchive::loadBinary<1|2|4|8>` (`0x27316c`, `0x283274`, `0x2744c8`,
`0x27f230`), the size-tag reader `0x27f4e0` and the 16-byte vector resize `0x28e4a0` into a
1M-entry ring, and dumps it on an absurd resize, on `/dump/scenelog.dump`, or when the game
first fetches the scene named in `/dump/scenelog.want`. `modes/scenelog_read.py <dump>
--file <scene.radium>` gives every read's file offset and caller. It also logs, once per
caller, every scene fetched, node path found and text set. The full grammar is in
`scene_write.py`'s docstring.

## A clip of our OWN: emulator-proven in a game (item 132, 2026-09-16)

A clip the card never shipped, played full screen by the mode. No stock clip is replaced
and no stock entry moves. Not hardware-proven.

**Where in-game clips come from.** Every one is played by NAME through one call,
`0x528a4(const char *name, int loop, const char *crop label)` (end-of-ball bonus:
`("EndOfBallBonus_BackgroundLoop", 1, "SquareCrop")` at `0x65c54`), on ONE scene: the
video bank `auto_loaded/60ed7e50` ("video.in_game_videos"). That scene maps each name to a
clip TWICE - in its library, where each clip is registered with its `scene.assets` path
and file size, and in its VideoSurface node's component, by bare id. 598 clips, sorted by
name bytes, all at `2.asset/<n>.asset`. Its four root frames are crops (Normal,
ScoreFrame, SquareCrop, LetterboxCrop), not clips.

**Build it:** `modes/mode_clip.py <stock 60ed7e50 scene.radium> <out dir> --name
KaijuRush_Clip --make "KAIJU RUSH"` (the writer is
`pinball_decryptor/plugins/stern/video_bank.py`) writes the bank with one more entry in
both maps (fresh id 602, path `2.asset/598.asset`) and renders a title-card clip with
ffmpeg: H.264 Constrained Baseline 3.0, 1360x768, 30 fps, silent, like most stock clips.
`--clip` takes your own instead, checked with ffprobe. A file that does not walk to its
last byte under the grammar is refused.
**Drive it:** `modes/kaiju_rush_own_clip.mode` - `clip_start` (and `clip_end`,
`clip_label`, `clip_layer`). `/dump/mode.clip` holding `<name> [layer]` plays any clip in
the bank, ours or stock, without starting the mode.

**Playing is not drawing (run 1).** `play_clip` alone decoded our clip to end of stream -
the guest asked for `2.asset/598.asset`, padvid served all 120 frames - and nothing
reached the glass, for our clip AND for a stock control. The display is immediate-mode:
whoever owns a scene draws it every frame. The game's own clip loop (`0x22d50`):

| Step | Call | Notes |
|---|---|---|
| play by name | `0x528a4(name, loop, crop label)` | 0 label = "Normal" |
| the player | `0x52694()` | the bank's scene player `[0x79ea04]`, created on first use |
| the surface | `0x526b8()` | its VideoSurface node |
| still playing? | `0x56b758(surface)` | 2 = playing, 3 once it has ended |
| advance | `0x3ba938(player, f32 ms)` | -> `0x4eea24(display, player, ms)`; the lap timer gives ms |
| draw this frame | `0x4eeb08(display, player, layer 0\|1)` | display = `[0x7db1a0 + 0x7c]`. The game calls it through `0x3ba964(player, 3)`, which takes the layer from the CURRENT EVENT and logs error 0x101 without one - every tick of ours |
| stop | `0x52b04()` | |

`hook.h` wraps it: `gz_clip_play`, `gz_clip_draw(ms, layer)`, `gz_clip_stop`; `mode.c`
draws every tick while the state is 2 and logs how long the clip ran and how many frames
it drew.

**Proven (run 2, in a game on the Tokyo map):** at mode.start, drawn on layer 0, our
clip filled the glass - captures at +1.5, +2.5 and +3.5 s read frames 31, 70 and 110 of
the title card - and the map came back after it: `playing after 0 ms`, `finished after
4035 ms, 243 frames drawn (surface state 3)`. 0 segv, 0 fatal, and the 6
`do_stack_unwind` throws every run has. **Layer 1 draws nothing visible**: a stock clip and
ours both played there (5207 ms, 3969 ms) and neither showed. Layer 0 covers the score
panel while the clip runs.

**The shipped path (run 3):** both scenes built by the CLIs (`mode_screen.py`,
`mode_clip.py`), a fresh `mode.so`, `kaiju_rush_own_clip.mode` unchanged. In one game: our
clip full screen at +1.5 and +3.0 s (frames 31 and 85, 4038 ms, 243 frames drawn); our
own screen "KAIJU RUSH / 1,000,000 A SHOT" once it ended; "+1,000,000" over the GAME'S own
ramp clip on a ramp shot; a stock clip (`Mothra_godzilla_attack20`) played and drawn
through the grown bank; "TOTAL 1,000,000" at the end and the screen hidden after. All 598
stock clip files had the same sizes and mtimes before and after; both scenes restored by
md5. 0 segv, 0 fatal.

**Open, measured:**
- **Draw order.** The game's own ramp clip draws UNDER the score panel and our screen;
  ours, drawn from the tick on layer 0, covers everything for its 4 s.
- **One surface.** Playing a clip replaces whatever the surface is playing - in run 3 the
  stock control cut the game's ramp clip (`290.asset`) short. `gz_clip_draw` checks the
  SURFACE's state, so a clip the game starts while ours runs is drawn by our loop too.
- **Not on a card.** The bank and the clip live in the games tree on p3, so a card needs
  `.sidx` records for both (item 129's delivery); nothing builds that yet.
- **Hardware decode.** Our clip claims level 3.0 at 1360x768, as the stock clips do; the
  i.MX6 VPU has not seen it.

## More sounds of a mode's OWN: carriers, the census, music (item 150, 2026-09-16/17)

Item 130 gave a mode one sound: a record appended to `image.bin` and one stock request's
descriptor re-pointed at it. Item 150 carries several (a start call, a shot call, an end call
and music), so it needed stock requests the game never plays, a way to stop a sound, and to
know how the game loops music. Addresses are Godzilla Premium 1.16 (Pro 1.15 in brackets).

```
sound_request_play(req)     0x43d2ac  (0x2a3108)   serial++, worker(req, &out, 0, 0, serial, 1, 0, 0, 0)
sound_request_play_nth      0x43d460  (0x2a32bc)   the same with r2 = 1 and n
sound_play_worker           0x43c898  (0x2a26f4)   9 callers; hookable (push, subs r7, r0, #0)
play_delayed(req, delay..)  0x43d300  (0x2a315c)   queues in 8 x 20 B at 0x7f86a8 (0x7b9310)
sound_request_active(req)   0x43da20  (0x2a387c)   any channel with +188 != 0 and +184 == req
sound_request_stop(req)     0x43db78  (0x2a39d4)   63 call sites; every such channel -> 0x43c2d0 (0 = all)
  stop by serial (+180)     0x43dc48               also cancels a delayed request
  stop a bus (+189 bit)     0x43e7a0               0x43e7ec stops all 8 buses; 0x43e80c all but req
channel release (mutexed)   0x43c2d0 -> 0x43c1e8   (Pro's "0x2a2044" is the latter)
channels                    0x7f7e34  (0x7b8a9c)   8 x 196 B: +180 serial, +184 u16 req,
                                                   +188 voices playing, +189 bus mask, +194 u16
callout(req)                0x18bafc  (0x187f44)   swaps in the JAPANESE variant (below), then plays
```

**The "city variant" map is the Japanese swap.** `callout` looks the request up in a
`std::unordered_map<u16,u16>` at 0x7ba680 (0x7a7268), filled at run time, only while
`RuleCities[+4 + 4 * current player]` == 1 (`0x3387f0` reads `cur_player`). Dumped live:
147 entries, each an English request -> its "... JAPANESE" sibling. What sets the flag is not
traced; the game's strings name a per-player language select.

**The descriptor key mask is per build, and Premium 1.16 keeps the engine's.** Under
0xE0001FFF 2628 of 2628 descriptor payloads name a real record (43 under 0xFC0003FF); Pro 1.15
is the other way round (2615 / 2615 under 0xFC0003FF). The Sound Test's node id IS the request
id (1318 / 1318 menu entries carry the request table's sid list), and request ids and names are
the same on both titles.

**The census (`sdk/sound_census.c`, RUN 1, Premium 1.16 stock):** 441 requests in ~11 minutes
of a played game, 77 distinct, and none of the 21 Japanese variants or the 6 stereo music
carriers; 9 swap sources played in English and no target. `sound_map.py` supplies the static
half (single sid, a record and sid no other request names, no constant call site).

**Music loops inside the engine, on one channel, and `sound_request_stop` ends it.** The game's
own main play music (req 67, 168.5 s of records) held ONE channel under one serial for 609 s;
`sound_play(84)` (TUNE 7, 42.7 s) held 64 s with `sound_active` 1 at 0.7 s, 51 s and 63 s, then
`STOP 84 rc 1` and `ACTIVE 84 0`; `STOP 918 rc 1` cut a 52.4 s record at 15.6 s. Not every
stereo request loops: 863 "TERRORLOOP1" (28.8 s) ended by itself at 29 s. Request word 4 is
0x101 on the music requests (66, 67, 75, 83, 84, 125, 127), 0x2 on effects.

**A bank with four sounds of our own (desk):** `mode_sounds.build_bank` on Premium 1.16's stock
bank, carriers 1251 / 1249 / 1133 (mono calls) and 125 (stereo music): 4 records appended, the
engine's integrity check passes for all 2534 stock sounds, and from fresh boots 0 stock records
moved, 5 sids re-pointed (the four carriers' plus sid 2490, which no request plays but which
named record 613 too), every other sid unchanged, decodes corr 0.9993 / 0.9950 / 0.9947 /
0.9985. An appended record must be LONGER than the record it copies
(`masterdir.plan_grow_records`), so a shorter sound is padded with silence past its carrier.

**In the emulator (2026-09-17, RUN 2A / 3 / 2B):** the ten record neighbours of the carriers
are frame-identical on the built and the stock Premium card; the carriers play our WAVs in
attract (Premium 0.988-0.997, Pro 0.989-0.997; the stock card's same firings <= 0.011); the Pro
1.15 census over two whole 3-ball games played none of the 27 carriers.

### Sound BUSES and priority: why music of our own was cut, and calls dropped

A mode in a game (RUN 3) played its start, first shot and end calls, but the next two shot
calls were silent and its music was restarted about every two seconds. The worker (0x43c898,
Pro 0x2a26f4) explains it. The request record is 20 bytes: +8 the sid list, **+16 priority,
+17 flags**; the table is 0x784844 (Pro 0x771530), its count the u32 at 0x61c28c (Pro 0x60bd34),
2030 on both. A descriptor names the BUS bits a sound may use, and each bus holds one sound.
The channel it plays on keeps a copy: +152 age, +180 serial, +184 request, **+186 priority,
+187 flags, +188 voices, +189 bus bits**.

```
for each channel playing on a bus the new request wants:
    new priority >  playing priority                      -> the bus may be taken
    new priority == playing priority and playing +187 & 1 -> the bus may be taken
    otherwise                                             -> that bus is removed from the choice
too few buses left -> return 0: the request is not played, and no error is raised
```

**Correction (item 150 fix-1, RUN 8):** that 0 does not reach the caller of `sound_play`.
`pm_sound(1249)` returned 1 (`shot call 1249 (shot 1) played`) while a start call at priority 4
held the speech bus, and the dump showed no 1249 channel and the capture no call (corr 0.036);
RUN 5's shots 3-4 under 1997 logged `played` the same way. So the return says the request was
accepted; only the dump or the capture says it was heard.

Read off the census channel dumps (RUN 1 Premium, RUN 2B Pro): bus 0x01 is music (66, 67, and
our 125 at priority 1, flag 1), bus 0x02 is speech (priority 2 or 3, flag 0: 934, 944, 2001,
2002, and our 1251 / 1249 / 1133 at 2, 0), buses 0x04-0x40 are effects (priority 2, mostly flag
1). So:

- **Music:** a new music request takes the music bus from the one playing (equal priority, flag
  set). The game's music polls and restarts itself, so our 125 and its 67 took the bus from each
  other: `music 125 had stopped - started again` 133 ms after the start and 11 times in 25 s; the
  chords of our loop restart at 101.8, 104.0, 106.6, 108.3, 109.2, 111.1, 112.1, 114.3 and 118.0 s.
- **Calls:** a speech request of equal priority with no flag is simply not played while another
  speech sound plays - our own previous call included (shots 3.2 s apart, a 3.67 s call).

**Measured with the channel dump in attract (RUN 4, `sound_fire_mode.c` `sound.dump`):** 1249
fired twice 1 s apart at priority 2 flag 0 kept serial 4 until it ended (3.76 s of PCM, the
second firing never took a channel); at priority 3 flag 1 serial 7 replaced serial 6 1.4 s in
(5.31 s of PCM). 125 with its flag cleared plays as `p1 f0 b01`.

**What a mode does about it (`pm_sound_priority`, `mode_file.c` at every start):** its music
keeps priority 1 without the flag (the game's music, equal priority, cannot take the bus back
until the mode stops ours), and its calls take the flag - RUN 5 with every call at priority 3,
then from RUN 7 shot calls at 3 (they cut the game's ordinary speech and the previous shot call,
not its priority-3 speech) and start and end calls at 4 (they cut that too). The game's own
priorities (Premium 1.16 request table): 1 = 114 requests (music), 2 = 1688, 3 = 88 (speech
among them: multiball intros, super jackpots, 1994-2023 the unnamed ball-start lines), 4 = 77,
5 = 1, 0 = 1, and 61 at 196-255 (requests 1-21, 24-61, 1161-1162). Carriers are requests the game never plays, so nothing it
plays changes.

**Emulator-proven in a game (RUN 5, Premium 1.16, `mode_sound4.so`, the channel dump):**

- Without a mode: 125 fired with the flag set lost the music bus to the game's 67 within 16 ms
  (`active 125: 0` at 6 s); with the flag cleared it held `p1 f0 b01` until stopped.
- **The game asks for its music again when another takes the bus, not on a timer:** after our
  stopped 125 the music bus stayed empty for 23 s with no switch hit, and 67 was back 77 ms after
  the next Maser target hit.
- SOUND RUSH (trigger file, three shots 4.7 s apart): 125 held the music bus under ONE serial from
  START to END with 0 restarts and was stopped at END; start, three shot and end calls each took
  the speech bus; the capture scores the calls at corr 0.898 / 0.627 / 0.900 / 0.746 / 0.989
  (the same WAVs 6 s before START <= 0.023) and the music in phase with ONE unbroken loop for
  16.0 s of 23.1 s, out of phase 0.0 s (RUN 3, before: 4.6 s in, 8.5 s out).
- SOUND RUSH with four shots 1.8 s apart: shot 2's call cut shot 1's; shots 3 and 4 were not
  played because the game's own priority-3 speech 1997 (flag 0) held the speech bus - a mode's
  call does not cut the game's priority-3 speech.

**RUN 6 and RUN 7 (2026-09-17, merged sources, both titles):** with every call at priority 3, a
mode started ~11 s after the ball launched lost its start call and first shot call under the
game's ball-start speech (Pro dump: 1319 "ATTENTION EARTHLINGS..." then 1995, both p3 f0). With
start and end calls at 4 (RUN 7): Premium 1.16 with the card object (`mode_file.c` alone) start
0.920, shots 0.615 / 0.820 / 0.669, end 0.989, music in phase with one loop 14.9 s of 23.0 and
out of phase 0.0 s; Pro 1.15 start 0.863, shots 0.687 / 0.829 / 0.681, end 0.989, music 15.5 s in
phase, 0.0 s out, the dump showing `1251 p4 f1 b02` and `125 p1 f0 b01` under one serial. Seen
in the dumps: a higher priority taking the speech bus from a lower one mid-sound three times
(938 p2 -> 1319 p3, 944 p2 -> 1997 p3, the mode's `callout_at` 1291 p2 -> our shot call 1249 p3),
and the game's music back on its bus at the first switch hit after the mode's END.

### Shorter sounds, and which music the game's features play (item 150 fix-1, 2026-09-17)

**A sound shorter than its carrier.** The appended record is never shorter than the carrier's
stock record. RUN 8 (Premium 1.16, 1 s calls, a 12 s music loop): without a length the 1 s start
call's record held the speech bus at priority 4 for its whole 4.61 s, and a shot call 2 s after
START got no channel (corr 0.036). With `sound_start 1251 1000` (`mode_file.c` stops the carrier
at ms + ms/8 + 120) the start call was stopped at +1.25 s, still heard whole (0.882), and the shot
2 s later took the bus (0.62 / 0.73). The build tiles music in whole repeats past the carrier: 12
s x 3 = a 36 s record on 125, and under a 45 s mode the capture followed ONE loop across both
seams and the record's own loop, 0.0 s out of phase.

**What the game's music features play** (desk, Premium 1.16): THEME MUSIC 66-70 / 73; Godzilla
multiball music selection and its Random pick 122, 73, 77, 88, 96, 101, 106; Terror of
Mechagodzilla its TERRORLOOPs 863-866 by name (effects buses). **The DJ Mixer (RUN 9, measured):**
both flippers held in attract, DJ MIXER, START; its ALL SONGS playlist is 40 requests of the
unnamed block 127-179 and it starts on 127 ("GODZILLA"); 125 and 126 are in no playlist. So the
one music carrier left is 125.

**Priority 4 over priority 3, seen (RUN 10):** with the game's 9.09 s line 2001 (p3 f0) on the
speech bus, a mode's start call took it within 49 / 83 ms (`2001 p3 f0 s9` -> `1251 p4 f1 s10`), and
its end call likewise (`2001 s14` -> `1133 p4 f1 s15`), heard at corr 0.83-0.98.
