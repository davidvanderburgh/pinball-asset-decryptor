...
| flag | meaning |
|------|---------|
| `extract` | Decode `image.bin` → per-sound WAV (`audio/`), copy out videos (`video/`) + images (`images/`) |
| `write` | Re-encode edited WAVs + patch replaced videos/images back into a **copy** of the card image (size-neutral, in place) |
...
Before any decode, the engine cold-derives the decode-params table for every cat-0 sound straight from the card by driving the firmware's own master-directory registration (`derive_params` — [emulator.py:631](../../pinball_decryptor/plugins/stern/spike2/emulator.py#L631)). Each entry is `{idx, body_off, length, chan, scale, ...}`. This is the slow step (~1–2 min for ~2000 sounds; big catalogs like D&D's ~10.5k sounds need more) so it's **cached by a fingerprint** of `game_real` + the `image.bin` master-directory region (`_fingerprint`/`_load_or_derive_params` — [engine.py:39](../../pinball_decryptor/plugins/stern/engine.py#L39), [engine.py:55](../../pinball_decryptor/plugins/stern/engine.py#L55)); re-runs are instant.
...
3. **Extract video / images / Decode audio** — `engine.extract_all()` ([engine.py:297](../../pinball_decryptor/plugins/stern/engine.py#L297)) drives phases 2–5. It opens the disk, streams `game_real` + `image.bin` out to a temp dir (`_extract_inputs` — [engine.py:109](../../pinball_decryptor/plugins/stern/engine.py#L109)), then:
   - **videos** first (quick file copies): every directly-stored video (sniffed by `ftyp` magic, so name/extension don't matter) is copied to `video/`, named from its scene's `scene.radium` (e.g. `Cowabunga_Background`), with a `manifest.txt` mapping output → card path (`extract_videos`/`_parse_radium` — [engine.py:159](../../pinball_decryptor/plugins/stern/engine.py#L159), [engine.py:131](../../pinball_decryptor/plugins/stern/engine.py#L131)).
   - **images** next: every loose image file is copied to `images/`, preserving the card's directory structure, with a `manifest.txt` (`extract_images` — [engine.py:236](../../pinball_decryptor/plugins/stern/engine.py#L236)).
   - **audio** last (the long step): boot the firmware, derive/load params, then decode every sound to `audio/idxNNNN.wav` — **across `min(cpu-2, 8)` spawned worker processes** (each boots its own emulator and writes WAVs directly), with a single-process fallback (`_parallel_decode`/`_serial_decode` — [engine.py:417](../../pinball_decryptor/plugins/stern/engine.py#L417), [engine.py:394](../../pinball_decryptor/plugins/stern/engine.py#L394)).
4. **Checksums** — `generate_checksums()` writes the `.checksums.md5` baseline so Write/Mod-Pack can tell which assets the user edited.

**Output layout:** `output_dir/audio/idxNNNN.wav` + `output_dir/video/<name>.mp4|.mov` (+ `manifest.txt`) + `output_dir/images/<card path>.png` (+ `manifest.txt`) + `.checksums.md5`.
...
`SternWritePipeline._run()` ([pipeline.py:94](../../pinball_decryptor/plugins/stern/pipeline.py#L94)) → `engine.write_image()` ([engine.py:933](../../pinball_decryptor/plugins/stern/engine.py#L933)) produces a **patched copy** of the card image, size-neutral and in place. The patch set is computed by the shared `_compute_patches()` ([engine.py:708](../../pinball_decryptor/plugins/stern/engine.py#L708)):

1. **Diff against the baseline.** Walk `assets_dir` for `idxNNNN.wav` (the index survives an Auto-transcribe rename like `idx0651 - text.wav`), and read `.checksums.md5`. Only sounds/videos/images whose MD5 differs from the baseline are re-encoded/patched — an untouched (or merely renamed) asset is skipped. Empty diff → a clear "nothing to write" error.
2. **Re-encode audio** (only if audio edits, and the build's codec is locatable). Boot the emulator, load params, and for each edited sound: verify the keystream recovery round-trips bit-exact for *this* sound before trusting it (`_recovery_valid` — [engine.py:1112](../../pinball_decryptor/plugins/stern/engine.py#L1112)) — a sound whose variant can't be reproduced exactly is **skipped, never patched blind**. Then `encode_sound` yields the new body bytes (`_encode_mono`/`_encode_stereo` — [engine.py:1166](../../pinball_decryptor/plugins/stern/engine.py#L1166), [engine.py:1174](../../pinball_decryptor/plugins/stern/engine.py#L1174)).
3. **Fit video / image** to the original's byte size (`_prepare_video_patches`/`_prepare_image_patches` — [engine.py:567](../../pinball_decryptor/plugins/stern/engine.py#L567), [engine.py:681](../../pinball_decryptor/plugins/stern/engine.py#L681)): a clip/image `<=` its slot is padded up (a trailing MP4/MOV `free` box, or trailing zero bytes for an image, both ignored by decoders); a larger one is re-encoded/re-compressed down to the byte budget; one that still won't fit is skipped with a warning.
4. **Flatten + apply.** Every patch resolves to absolute `(disk_offset, bytes)` writes via `Ext4Reader.disk_ranges` (audio bodies inside `image.bin`'s inode; videos/images over their own inode). `write_image` copies the original image to the output, then applies the writes in place (`_apply_writes` — [engine.py:924](../../pinball_decryptor/plugins/stern/engine.py#L924)). A video/image-only write **skips the firmware emulator entirely**.
...
- **Extract:** `SternDirectSsdExtractPipeline._run()` ([pipeline.py:126](../../pinball_decryptor/plugins/stern/pipeline.py#L126)) confirms the device is a Spike card + resolves its ext partitions (`engine.device_partitions` — [engine.py:970](../../pinball_decryptor/plugins/stern/engine.py#L970)), then calls `extract_all` with `open_disk=lambda: RawDeviceFile(dev)`. Everything downstream is identical to the file path (game_real + image.bin still stream to a temp dir, then decode).
- **Write:** `SternDirectSsdWritePipeline._run()` ([pipeline.py:178](../../pinball_decryptor/plugins/stern/pipeline.py#L178)) → `engine.write_device()` ([engine.py:1009](../../pinball_decryptor/plugins/stern/engine.py#L1009)) computes the *same* patch set via `_compute_patches` and applies those exact byte ranges to the card with a writable `RawDeviceFile` (no image copy).
...
**You cannot make a replacement longer than the original sound.** The encode is size-neutral: the new body is patched back into `image.bin` at the original `body_off` with the original byte length (the packed container's master directory uses absolute offsets, so resizing one sound would strand every following offset). Concretely, `_encode_mono`/`_encode_stereo` fit the replacement to `p["length"]` (the original sample count) via `_fit` — **a longer track is trimmed; a shorter one is zero-padded with silence** ([engine.py:1064](../../pinball_decryptor/plugins/stern/engine.py#L1064), [engine.py:1166](../../pinball_decryptor/plugins/stern/engine.py#L1166)). The replacement is also resampled to 44100 Hz (`_load_wav` — [engine.py:1049](../../pinball_decryptor/plugins/stern/engine.py#L1049)) and amplitude-limited into the codec's range (`_amplitude_fit` — [engine.py:1073](../../pinball_decryptor/plugins/stern/engine.py#L1073)). So a longer track won't *break* the game — it's safely trimmed — but it will be **cut off** at the original's duration. This is summarised for the user by `audio_length_note()` ([manufacturer.py:138](../../pinball_decryptor/plugins/stern/manufacturer.py#L138)). The same size-neutral principle governs Replace-Video/Image (fit to the original's byte size).
...
- **Video:** H.264 in an MP4/QuickTime container, stored verbatim as `.asset` files. Extract sniffs the `ftyp` magic and names each from `scene.radium` scene-element identifiers ([engine.py:159](../../pinball_decryptor/plugins/stern/engine.py#L159)). Replace-Video patches the new clip over the original `.asset` inode, size-neutral: padded with a trailing `free` box if smaller, re-encoded down to the byte budget if larger, skipped if it still won't fit (`_pad_isobmff`/`_fit_video_payload` — [engine.py:459](../../pinball_decryptor/plugins/stern/engine.py#L459), [engine.py:528](../../pinball_decryptor/plugins/stern/engine.py#L528)). `video_length_note()` ([manufacturer.py:143](../../pinball_decryptor/plugins/stern/manufacturer.py#L143)).
- **Images:** loose `.png` UI art. Extract preserves the card directory tree; Replace-Image scales the replacement to the original's pixel dimensions and patches it over the inode size-neutral (pad with trailing zeros, or re-compress to fit — `_pad_image`/`_fit_image_payload` — [engine.py:600](../../pinball_decryptor/plugins/stern/engine.py#L600), [engine.py:643](../../pinball_decryptor/plugins/stern/engine.py#L643)). `image_note()` ([manufacturer.py:148](../../pinball_decryptor/plugins/stern/manufacturer.py#L148)).
...
- **No bundled per-title data.** Every sound's params + keystream derive from the card's own `game_real` + `image.bin` at runtime; params are cached by a fingerprint of those two ([engine.py:39](../../pinball_decryptor/plugins/stern/engine.py#L39)). A brand-new Spike 2 title works as soon as its card is recognised — *if* `locate.py` can find its firmware addresses.
...
- **Re-encode self-test gates every patched sound.** `_recovery_valid` re-decodes a freshly re-encoded sound's own audio and requires it bit-exact over the first blocks before Write trusts it; a sound that fails is skipped (left unchanged), never written blind ([engine.py:1112](../../pinball_decryptor/plugins/stern/engine.py#L1112)).
...
- **Parallel decode needs guarded entry points.** Decode fans out across spawned processes (`multiprocessing` "spawn"); the app's entry points are guarded so a worker re-import doesn't re-launch the GUI. A stalled pool falls back to a single process ([engine.py:417](../../pinball_decryptor/plugins/stern/engine.py#L417)).
- **Direct-SD writes need no disk-offline.** The patched bytes are all in the ext4 partition (no Windows volume there), so an admin raw handle writes them in place — unlike SSD plugins that take the disk offline / mount via WSL. The signature check refuses a non-Spike drive before any write ([engine.py:970](../../pinball_decryptor/plugins/stern/engine.py#L970)).
...
- **`engine.py`** ([engine.py:1](../../pinball_decryptor/plugins/stern/engine.py#L1)) — orchestration: `extract_all`, `write_image`/`write_device`, `_compute_patches`/`_apply_writes`, `device_partitions`, video/image extract + size-neutral fit, the encode helpers + re-encode self-test.
...
- **`tests/test_stern_video.py`**, **`tests/test_stern_image.py`**, **`tests/test_stern_direct_sd.py`** — size-neutral padding/diff, the Replace-Video/Image patch helpers, and the Direct-SD raw-device + MBR-helper + write-equivalence tests.
