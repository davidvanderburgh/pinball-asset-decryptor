...
# and item 131's scenelog.so (the scene.radium read recorder).
...
cp "$RIG/modes/hook.h" "$RIG/modes/padmode.c" "$RIG/modes/mode.c" "$RIG/modes/scenelog.c" "$PAD_STAGE/"
for so in padmode mode scenelog; do
