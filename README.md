...
5. **Replace Audio tab** *(file-based plugins)* — swap a game's music /
   ...
   **Rename…** corrects a slot's name in place; the name is remembered
   ...
   slot keeps its Type bucket. On Stern extracts the Rename dialog also
   suggests the game's **own Sound Test menu names** (from the
   `sound_test_names.csv` the extract writes): play a number on the
   machine's Sound Test menu, then pick that entry for the slot that
   played it.
