...
- **Substitutions must be SIZE-NEUTRAL — the PCK has an encrypted directory of absolute offsets (RE-confirmed).** BoF's May PCK is a **stock Godot 4.5.2 v3 pack**. The stock `file_count` at header offset 96 is 0 (why GDRE refuses it and why `may_extractor` marker-scans inline sidecars instead) — but the engine is unmodified `PackedSourcePCK::try_open_pack`, which in v3 reads a **u64 `dir_offset` at header offset 32**, seeks `pck_start+dir_offset`, reads `file_count` cleartext, then reads the real file directory through **`FileAccessEncrypted`** (AES; FAE magic GDFE→EBOF). Each entry holds an **absolute offset** (`pck_start+file_base(104)+ofs`), size, md5[16], flags. The running game locates every resource by these stored offsets, so **any net size change strands the offsets of all later files → the engine reads boot resources at the wrong place → black screen** (even editing a gameplay-only asset, because the shift corrupts everything after it; the MD5 "validated" gate still passes — it only checks USB-transfer integrity). Our extractor sidesteps the directory (marker-scan) so a round-trip looks clean, but the real engine does not. `may_packer` therefore makes every substitution **size-neutral**: zero-pad the replacement back to its exact original byte length (the resource loader ignores trailing bytes; QOA stops at its frame count) so nothing shifts and the encrypted directory stays valid; a replacement **larger** than the original raises `PackerError` (can't grow without shifting). Audio never hits that error because `encode_wav_to_sample` **auto-trims** a too-long clip to fit the original payload's byte budget (`_max_qoa_samples`) and warns via `log_cb` — only images/other assets can trip the `PackerError`. Unchanged fonts must also stay verbatim (don't re-wrap on a compressed-byte compare). Verified by `test_pack_is_size_neutral`, `test_pack_rejects_larger_replacement`, `test_pack_leaves_unchanged_fonts_verbatim`; a clean local proxy is "repacked binary == original except inside the edited file's footprint; `pck_size`/`dir_offset`/trailer unchanged." **Open work:** growing a file needs a full directory rewrite (decrypt → update offsets/sizes/md5s → re-encrypt → repoint `dir_offset`), blocked on the FAE AES key (derives from `script_encryption_key` @0x5ec5ba0 via an unrecovered `Security::TOKEN` transform). See memory `reference_bof_pck_alignment`.
