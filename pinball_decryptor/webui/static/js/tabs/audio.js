...
function AdvancedModal({ data, onClose }) {
  ...
  // macOS has no SD card size control on the Write tab (write.card_size_supported)
  const plat = (state.write || {}).platform;
  const mac = plat ? plat === "darwin" : /Mac/.test(navigator.platform || "");
