...
**A sound that re-encodes wrong is a sound that DECODED wrong — don't chase the encoder.** The model above assumes output sample `i` reads one body word (`i + delta`). Some sounds appear to break it: on Deadpool Pro 1.16 idx7146 all 14 blocks fit `200*b + 2*i` bit-exact instead (only even words read, consecutive blocks sharing them). That mapping is **not a real codec mode** — it is an artefact of a broken decode. The recovered keystream and the decoded samples both come from the *same* emulation, so agreeing with each other only proves internal consistency, never that the audio is right. The tell is acoustic: those sounds are stationary noise. Measured spectral flatness ≈ `0.38` with envelope ratio ≈ `1.04` and zero quiet frames, against `0.001–0.07` / `1.6–3.2` / `0.07–0.34` for every healthy sound in the same sample (Godzilla 1.13 idx14, Star Wars 1.30 idx118 — the "exceptions" on long-shipped titles are the same thing). See the catalog-tail noise investigation: Deadpool 1.16 decodes 0% dead below idx 4000, 28.5% at 4000–4999 and **100% above idx 5000**, versus TMNT 1.58 at 0.7% and Jaws 1.01 at 0.0%.

So the re-encode is not independently broken and needs no "stride-aware" encoder — that would be actively wrong, since every slot is exactly `2*length` bytes (catalog-wide `gap/2len` median 1.00) and writing two words per sample would overrun the neighbouring sound. Fix the decode and the round-trip follows. Meanwhile Write is safe by construction: `_recovery_valid` re-encodes each sound's own audio first and skips any that doesn't reproduce bit-exact ("re-encode isn't bit-exact for this sound's codec — skipped, left unchanged"), so a noise-decoding sound is left alone rather than written over.
...
**The record count is captured where it is live, not at the malloc return.** The count used to be read as `r5` at the malloc-return PC. That is only true for builds whose `*24` size computation lands in a *spare* register (TMNT 1.58: `add r6, r8, #0xf`). A newer compiler reuses the count register itself (`add r5, r7, #0xf ; bic r5, r5, #0xf`), so at that PC `r5` holds the **aligned byte size** — exactly 24× too high — and the derive chains through tens of thousands of garbage records. That is the "Deriving codec parameters" hang behind a field report (Deadpool Pro 1.15/1.16), and **a static sweep of the 35-card library found 8 affected images**, not one: Deadpool Pro 1.16, Jaws 1.01, Mando 1.44, Metallica 1.03, Munsters 1.27, Star Wars 1.30, Sword of Rage 1.18, TMNT Pro 1.59. Severity scaled with catalog size, so the big catalogs hung outright while smaller ones ground through ~24× the work (measured: TMNT Pro 1.59 real count 2067 vs 49616 read; Star Wars 1.30 2666 vs 63984).

`locate._find_internal_pcs` now also reports **`MASTERDIR_COUNT`** (the PC of the `add rD, rN, rN, lsl #1` that begins the `*24`) and **`COUNTREG`** (`rN`) — the one point where the count is provably live on every build shape — and `derive_params` captures it there. Two backstops remain behind it: `_md_record_count` recovers the count from the malloc the import stub actually served (`size // 24`, exact — the 16-byte pad is < 24) when no count site was located, and `_accept_masterdir_malloc` rejects an insane count (`MAX_RECORDS`, shared with the per-category gate) so the fail-fast watchdog stays armed and a genuinely unmapped build bails within ~1–2 min into the "could not map this firmware build" path instead of hanging. Note the sanity gate alone was never sufficient: Star Wars' bogus 63984 sits *under* the bound.
...
- **What may go on the card untouched.** An oversized clip avoids that byte-budget crush by *growing* its slot through the ext4 driver instead (`_prepare_video_patches` → `core/ext4_grow`), and that copy is byte-for-byte — which means whatever the user picked in the file dialog lands on the card. The machine's decoder is an i.MX6 VPU, not a desktop player: give it a Matroska container, HEVC, a 10-bit / 4:2:2 stream, or a resolution the scene never sized a surface for, and it demuxes far enough to **play the sound over a black picture**, with nothing said at build time. So `_intact_copy_source` ([engine.py:2013](../../pinball_decryptor/plugins/stern/engine.py#L2013)) gates the verbatim copy on the file actually being a drop-in for the clip it replaces — same container brand (the extract extension records whether the card's own clip was `qt  `-branded), H.264, 8-bit 4:2:0, the slot's own resolution and frame rate, and an H.264 profile no higher than the slot's own (`profile_rank` — the slot's clip is a decode *ceiling*, and pinning re-encodes to it while letting the intact path ship anything above it would have been the one hole left). The staged copy is the yardstick, since Replace-Video already format-matched it to that exact slot. A source failing any check falls back to that staged copy, logged as ONE `info` line naming the reason — not a warning, because with conversion left on the format-matched copy going on IS the plan (a tester read the old black-picture warning as a build failure). It still grows to full size, so the only thing lost is the re-encode the user's own file would have avoided. This also makes the list's **Convert** column honest: a row that says `Re-encode` now really is re-encoded.
- **A wrong wrapper is repackaged, not re-encoded.** a tester's replacement was H.264 at the slot's resolution and frame rate but written as `.mp4` for a QuickTime slot — one property away from a drop-in, and re-encoding the picture to fix the container costs a generation of quality for nothing. `stage_replacement` now tries `_remuxable` ([video_slots.py:241](../../pinball_decryptor/core/video_slots.py#L241)) → `remux_video_to` ([video.py:847](../../pinball_decryptor/core/video.py#L847)) first: a stream copy into the slot's container, so every coded frame survives bit-for-bit and only the container boxes are rewritten (the slot's audio shape still applies — a silent slot gets `-an`). The build gate still refuses the user's raw `.mp4`, but the staged copy it falls back to is now lossless. The Convert column says `Repackage` for it. A failed remux (an ffmpeg that won't take the stream) falls through to the full re-encode rather than failing the stage.
- **The copy-through gate matches the drop-in gate.** `_already_matches` used to compare container *extension*, codec, resolution, frame rate and alpha only — so with conversion ON a 10-bit clip at the slot's exact geometry, or a `.mov` an encoder wrote with an MP4 brand, was copied through verbatim and reached the card unplayable, where the build gate's fallback (`_same_bytes`) could only report it after the fact. It now shares `_stream_matches` ([video_slots.py:173](../../pinball_decryptor/core/video_slots.py#L173)) — pixel format and profile ceiling included — and reads the real ISO-BMFF brand of both files (`_same_container`), so those get re-encoded or repackaged instead.
- **Re-encodes copy the stock clip's H.264 profile.** The clip already on the card is proof of what the machine will decode, so `_h264_profile_args` ([video.py:531](../../pinball_decryptor/core/video.py#L531)) pins `-profile:v` (and `-level`, when the output keeps the slot's dimensions) to the original's rather than letting libx264 default to High — the same black-picture-with-sound failure mode an embedded decoder gives for a profile above what it supports. `VideoInfo` carries `profile`/`level` for this, from ffprobe or the ffmpeg banner fallback.
- **What the clips actually are (census, 2251 clips / 5 titles).** Parsed straight out of the card images (`moov`>`trak`>`mdia`>`minf`>`stbl`>`stsd`>`avc1`>`avcC`), not taken on trust: **every clip is H.264 (`avc1`), 8-bit 4:2:0 — 163/163 on Led Zeppelin 1.22 LE, 658/658 Godzilla 1.13 LE, 650/650 Jaws 1.01 LE, 99/99 Deadpool 1.14 LE, 681/681 John Wick 1.01 LE.** The i.MX6 decodes H.264 in a hardware VPU and nothing else, so a ProRes or HEVC replacement demuxes far enough for the sound to play while the picture stays black — the exact failure a tester reported, from a ProRes `.mov` copied on as-is. Three things vary *within* a single title and must therefore be read per SLOT, never assumed: **profile/level** (Baseline 3.0, Main 3.1, Main 3.2, Main 4.1, High 2.1, High 3.2 all appear), **geometry** (1360x768 alongside 520x294 on Godzilla, 454x256 on Deadpool, 1920x1080 on John Wick), and **audio** — nearly every clip is silent and the game supplies its own sound, but Deadpool carries an audio track on 7 of its 99, so "Spike 2 videos have no audio" is *not* a safe rule. A silent slot given a replacement that keeps its source's soundtrack gets that soundtrack played over the game, so `transcode_video_to` matches the slot's audio shape (`-an` for a silent slot) and the Replace-Video list flags an as-is pick that would add one. `core.video.dropin_spec` / `dropin_ffmpeg_command` surface the whole target through the list's right-click **What this slot needs…**, for users who would rather encode their own clips than let the app convert.
...
- **A grow job's source must outlive `_compute_patches`.** Oversized videos and the blip-free firmware don't go in the flat write list — they're copied in by the ext4 driver *after* `_compute_patches` has returned and run its `finally`. Video sources are the user's own files, so they survive; the rebuilt firmware is something the Write *builds*, and it used to be built into the scratch dir that `finally` deletes. `ext4_grow.grow_files` then dropped the job silently (its source no longer existed) and returned 0, which the caller read as an ordinary growth failure. The card shipped with the firmware's `.sidx` record already rewritten to the grown size + cave digests, no firmware to match it, and — because the blip-free path deliberately skips the in-place validator bypass, expecting it to ride inside that firmware — no bypass either. Reproduced end to end on the reporter's own title, a real James Bond Premium 1.06.0 card (Stern ships one build for Premium and LE and names the card `james_bond_le`, so the on-card paths read `le`): `game` 8,436,832 bytes on disk against a record claiming 8,441,856, digests mismatched, `push {r4-r7,sl,lr}` still at the validator entry. Every blip-free build from v0.94.0 (which introduced the grow) to v0.102.0 was affected on hosts that *have* the Linux driver — the ones that don't fall back to the standard build and were fine. The rebuilt firmware now gets its own directory, handed to the caller as `grow_plan["cleanup"]` and removed by the caller once the copy has happened, and a missing grow source is a loud error rather than a silent drop.
- **The validator bypass is idempotent on purpose.** It overwrites the validator's `push {..., lr}` prologue, which is the very thing `find_validation_exec`'s prologue check looks for — so an already-patched firmware would otherwise "lose" its validator, and (since `carries_validator` still sees the on-LCD strings) get reported as unreachable. The entry check therefore accepts our own `bx lr` as well, which keeps a rebuild from an already-modded image, or a second Direct-SD write onto a modded card, from warning about a card that is correctly patched. The CRC32 evidence still selects the function, so this never widens what can be matched.
- **The validator is located two independent ways, and a miss has to say so.** `find_validation_exec` first picks the function carrying several inlined CRC32-`0xEDB88320` loops ([valpatch.py:1](../../pinball_decryptor/plugins/stern/valpatch.py#L1)). That is a signature of how the firmware was *compiled*, not of what it does, so a build that factored its CRC32 out of line would match nothing — hence a second locator, `_by_shape`, keyed on the routine's measured shape: 1537–1547 instructions, exactly 22 callees, 92 backward branches, 269 loads, 198 stores, 156 compares, one caller. Profiled across the whole vendor library, the two select the **same single function on all 35 cards** that carry the validator, and the shape locator raises no false positive on any of them; it is all-or-nothing (every band must hold, and two candidates means refuse), because patching four bytes into the wrong function is how the pre-v0.94.0 cave landed on Elvira's node board table.
- **Jaws LE carries no validator — established positively, not assumed, and confirmed on two builds.** Jaws is the only title in the 37-card library where neither locator matches, and the question was whether the routine is missing or merely unrecognised. Three findings settle it. (1) *The binary is the same build lineage:* take a function every card shares and its address-normalised instruction stream matches Jaws exactly (307 of 303 8-grams, landing on the corresponding address), so a null result there carries information. (2) *The body is absent everywhere:* the validator's ~1545-instruction body, normalised the same way, scores **zero** matching 8-grams — not just in `game`, but across all 4,437 files in both partitions of the card. Run against other cards the identical sweep lands ~1300 hits squarely on their validator, so the method finds the routine when it is there. (3) *Its private data is absent:* the routine's progress-string pool (`SS: %u:%u`, `GE: PD`, `GE: %5.2f%%`, and the `CE:` / `ZK:` / `SF: %u:%u:%u` family) is data the compiler must emit for it, and it is present on the 35 cards where we locate the validator and on neither Jaws build.

  **1.02.0 is the confirmation**, and the reason to treat 1.01.0 as deliberate rather than a one-off build accident: a later release of the same title, a substantially different binary (`.text` grew ~10%, 6.16MB → 6.80MB), with three shared control functions still matching exactly (307 / 268 / 155 hits) while the validator body still scores **zero** and the pool is still absent. A rebuild that size is a fair chance for a merely-unrecognised routine to resurface.

  What misled the earlier reading is that Jaws still ships the six `#N ... UPDATE SD CARD` messages — both builds do. Those turn out to be worthless as evidence: byte-identical on all 37 cards, and on *every* card — including ones with a live validator — nothing references them directly, because they are reached through a data table. A string the linker kept is not a check that runs. The CRC32 machinery Jaws does contain is the ordinary table-driven zlib helper shared by 14 general-purpose callers, the same set as on every other card; its one CRC-using function without an obvious counterpart handles node-board `.hex` firmware updates.

- **`carries_validator` therefore tests evidence, not our own signature.** Two independent markers — the private string pool, and inlined `0xEDB88320` sites in `.text` — each split the library the same way (present on the 35 located cards, absent on both Jaws builds). *Either* marker alone answers yes, so a build that stops inlining its CRC32, or renames its progress tags, is still reported as armed; absence is asserted only when both are missing. The safety property is what the markers are for: a firmware carrying a marker whose routine neither locator can pin is `unlocated`, and the Write says so loudly. `bypass_status` returns `bypassed` / `absent` / `unlocated`; only `unlocated` warns — on the build log at `error` level and in the Write Complete dialog. Currently 35 cards report `bypassed`, the two Jaws builds report `absent`, and none report `unlocated`.
...
- **The card window is mapped in one piece, not paged.** `mem_map_ptr` backs the whole offset-identity `image.bin` window with a single zero-copy region over a private copy-on-write `mmap` (so a guest write still lands in a private page and never reaches the card file). Paging it in 4 KiB pieces made every later guest memory access search an ever-growing pile of tiny unicorn regions, so per-record derive cost was linear in mapped-page count and the derive was **quadratic in catalog size**: Deadpool Pro 1.16 went 11.2 ms/record at record 100 to 54.5 by record 3000, 18.8 min for the full 8175. One region gives a flat 11.0 ms/record, **2.58 min**, peak RSS 81 MB, and bit-identical params (8175/8175 rows). Chunked middle grounds are strictly worse — they copy: 64 KiB bought 1.36x for 357 MB, 256 KiB 2.15x for 577 MB. Any platform where the mapping doesn't take keeps the paging path ([emulator.py:436](../../pinball_decryptor/plugins/stern/spike2/emulator.py#L436)). **The derive is strictly sequential and cannot be parallelised** — each record's band build starts from the state the previous record left, so replaying a late record from the initial state produces no object at all; per-record progress is reported instead.
- **The chain replay must not write outside the record array.** `_drive_step` hands each master-directory record back to the band build by writing it at `r9 - 8`. That is the record's own slot on the validated build, where r9 at the band-loop head is the record cursor — but newer builds open that loop head with `and r1, sb, #1`, so r9 is a *packed data word* and `r9 - 8` is a pseudo-random 32-bit address. The replay then mapped a page and scribbled 24 bytes of live guest memory **once per record**, so the bug scales with catalog size: Deadpool Pro 1.16 (8175 sounds) took a hit at record 4714 and every record after it derived its predictor (`obj+0x18`) from corrupted state — 42% of the extract decoded to stationary noise. The write is now bounded to the record array (`_record_write_addr`), where it is a no-op because the record is already there ([emulator.py:180](../../pinball_decryptor/plugins/stern/spike2/emulator.py#L180)). **Ground truth for this class of bug is the firmware's own uninterrupted loop:** hook `BANDOBJ` and let `MASTERDIR_DECODE` run (keeping the find-skip) instead of replaying, then diff the objects — that is what isolated a single wrong field out of eight.
