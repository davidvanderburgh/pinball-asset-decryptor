 ...
 *   padmode.node    "<0|1> [<scene id>:]<path>" / "2" / "3"  show or hide a scene node by name; hold it (item 131)
 *
...
/* ---- item 131: a scene node of our own, shown and hidden by name ------------------
 * The helpers live in hook.h (gz_scene, gz_find_node, gz_node_visible, gz_node_text),
 * shared with mode.c.
 *   padmode.node "1 [<scene id>:]<path>"  find and show   "0 ..."  find and hide
 *                "2"  hold the last value, re-applied every tick     "3"  stop holding
 * With no scene id the attract scene is searched; in a game the glass is the HUD
 * scenes (32e6ae28 carries the mode slide-outs). */
static void *our_node;
static int our_node_on, our_node_hold;
static char node_arg[128];

static void *find_node(char *path)
{
    char *colon;
    void *node;
    for (colon = path; *colon && *colon != ':'; colon++) ;
    if (!*colon) return gz_find_node(gz_scene(0), path);
    *colon = 0;
    node = gz_find_node(gz_scene(path), colon + 1);
    *colon = ':';
    return node;
}

static void node_trigger(void)
{
    char m[260];
    long n;
    int fd = open("/dump/padmode.node", O_RDONLY), op;
    char *s;
    if (fd < 0) return;
    n = read(fd, node_arg, sizeof node_arg - 1);
    close(fd);
    unlink("/dump/padmode.node");
    node_arg[n > 0 ? n : 0] = 0;
    for (n = 0; node_arg[n]; n++) if (node_arg[n] == '\n' || node_arg[n] == '\r') { node_arg[n] = 0; break; }
    op = node_arg[0] - '0';
    for (s = node_arg + 1; *s == ' '; s++) ;
    if (op == 2 || op == 3) {
        our_node_hold = op == 2;
        snprintf(m, sizeof m, "[trigger] node hold %s (node %p, value %d)\n",
                 our_node_hold ? "on" : "off", our_node, our_node_on);
        hk_logs(m);
        return;
    }
    if ((op != 0 && op != 1) || !*s) return;
    our_node = find_node(s);
    our_node_on = op;
    gz_node_visible(our_node, op);
    snprintf(m, sizeof m, "[trigger] node \"%.100s\" -> %p vtable %p: visible %d\n",
             s, our_node, our_node ? *(void **)our_node : 0, op);
    hk_logs(m);
}
...
static void on_tick(unsigned *r)
{
    ...
    (void)r;
    if (our_node_hold && our_node) gz_node_visible(our_node, our_node_on);
    if (++ticks % 30) return;
    poll_triggers();
