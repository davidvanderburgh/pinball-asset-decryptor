...
| batman | ✅ static (swelf.py), live-verified | — | — | ✅ VILLAIN VISION window: node 24's "3 LCD INSERT", own `[villain vision]` window, clips PLAY (lossless webp), block commands CYCLE clip-by-clip (`assets 919-928 · showing 921`); shim answers the 0x90 status poll (echoes the playing asset — live-graded: 250 ms re-sends gone, no node alerts); `lcdring.py` reads the command transcript live or from `padlcd.last`. ⚠️ The game renders the villain TVs ITSELF on real hardware (lcdnode is an LPC1113 — cannot decode video) and that render path still aborts on our rig (4 channels stop at 0 frames, one EGL surface ever) — the window mirrors the wire until that player runs (items 82/83) | 2026-08-25 |
