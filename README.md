...
| Stern Pinball (Spike 1) | **WSL2 itself**, for the **Emulate** tab only *(a patched ARM `qemu-user` plus a CUSE hardware model, so the game runs unmodified under an emulated version of its own board set)* | build tools for the emulator's one-time compile: `meson`, `ninja-build`, `libglib2.0-dev`, `pkg-config`, `flex`, `bison`, `gcc`, `libfuse3-dev` | The rig ships with the app in [`tools/spike1_emu`](tools/spike1_emu) and is not yet wired into **Install Prerequisites** — the first Start builds the patched emulator and the device model itself and names exactly which apt package is missing if the distro doesn't have one yet, rather than failing silently. Extract and Write need no WSL at all — `image.bin` is plaintext PCM, decoded and patched directly on the host. |
