...
- **Needs a working C toolchain in WSL** (the shims above are compiled at runtime). `_ensure_compiler` ([pipeline.py](../../pinball_decryptor/plugins/jjp/pipeline.py)) proves it by actually compiling a throwaway file *before* the extract starts, and installs `gcc libc6-dev` itself if that fails. `which gcc` is deliberately not the test: Ubuntu's `gcc` package only **recommends** `libc6-dev`, so a WSL installed without recommended packages has the driver with none of its headers and every compile dies on `stdio.h: No such file or directory` — five phases in, after the extract and the dongle handshake (a tester's Sonic extract, 2026-07-28).
...
**Convert** (`_phase_convert_standalone`, [pipeline.py:4797](../../pinball_decryptor/plugins/jjp/pipeline.py#L4797)): `e2fsck -fy` the modified image, then `partclone.ext4 -c` → `pigz`/`gzip` → `split` back into `sda3.ext4-ptcl-img.gz.*` chunks (split size matched to the original first chunk). **Build ISO** (`_phase_build_iso`, [pipeline.py:2879](../../pinball_decryptor/plugins/jjp/pipeline.py#L2879)): `xorriso -indev <orig> -outdev <new> -joliet on -boot_image any replay` to clone the original ISO's boot structure (MBR/El Torito/EFI/Syslinux), then `-find … -exec rm` the old chunks and `-map` the new ones in. **`-joliet on` is load-bearing**: `-indev/-outdev` writes Rock Ridge only without it, so the rebuilt ISO lost the long-name tree Windows reads — `live/vmlinuz` showed as `LIVE/VMLINUZ`, `sda3.ext4-ptcl-img.gz.aa` as `SDA3_EXT4_PTCL_IMG_GZ.AA` — and the stick maker refused the app's own output (`usbstick.iso_has_joliet`), while a hand-made stick stopped in GRUB on `file '/live/vmlinuz' not found` (PAD-165). The result is checked for that tree (`_verify_iso_joliet`) and verified to actually differ from the original partition data — [pipeline.py:3022](../../pinball_decryptor/plugins/jjp/pipeline.py#L3022).
...
### USB install stick (in-app)

`capabilities.flash_image` reuses the shared Build / flash button + dialog, relabelled via the `flash_*` wording attributes (registry) to "Build / make USB install stick…" — but JJP's `make_flash_pipeline` is **not** a dd write: `UsbStickPreparePipeline` ([usbstick.py](../../pinball_decryptor/plugins/jjp/usbstick.py)) formats the stick FAT32 in an MBR table (label `JJPUSB`, the name JJP's own Mac PDF uses), mounts the ISO read-only, copies its files onto the stick, size-verifies every file, then flushes + ejects. Per-OS: Windows = PowerShell storage cmdlets (`Clear-Disk`/`New-Partition`/`Format-Volume`, partition capped at 32 GiB — Windows can't create larger FAT32 — with a Rufus pointer if the ISO wouldn't fit) + `Mount-DiskImage`, elevated in-process (launcher.vbs) or via one `RunAs` prompt — and when the cmdlets fail (the Storage-WMI provider can keep serving a stale post-`Clear-Disk` view where `New-Partition` says "Not enough available capacity" through every `Update-Disk` retry), the format is redone via a `diskpart` script (VDS, no shared cache; size from `Win32_DiskDrive`, explicit drive letter from `DriveInfo`); the boot/system-disk guard is re-checked in the fallback, and it is skipped entirely for the safety refusal or a declined UAC prompt; macOS = `diskutil eraseDisk MS-DOS JJPUSB MBR` + `hdiutil attach` (no root needed); Linux = `pkexec` parted/mkfs.vfat + `udisksctl` loop-mount/power-off. A directory `device_path`/`iso_path` short-circuits format/mount for unit tests. The drive picker uses the strict `flash_target_kind = "usb_stick"` kind (core.drives `_pick_usb_stick`): only a stick-sized external that does not look like an SSD/HDD (model regex + NVMe bus) is ever auto-selected, obvious disks are hidden from the dropdown (with the show-everything fallback so a manual pick stays possible), and when nothing qualifies the dialog selects **nothing** instead of falling back to the first drive.
