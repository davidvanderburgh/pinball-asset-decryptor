"""Capture the Emulate tab while the game shows its mains lock (PAD-173).

    python scripts/shot_pad173.py <out.png>

A variant of shot_pad149.py (same PrintWindow, same DPI-unaware capture, same
settings backup).  Power is set to "50 Hz mains, US machine" and the panel is
handed the status the rig reports while a US board sits on 50 Hz mains with
the auto-advance helper standing down: ``state=techalerts auto=0
auto_result=mainslock``.  Run against a build from before PAD-173 the same
status reads as plain Tech Alerts, which is the pair's own control.  No WSL,
no emulator: the pollers are stopped and the status is synthetic.
"""
import ctypes
import os
import shutil
...
OUT = os.path.abspath(ARGS[0] if ARGS else "emulate-mainslock.png")
REPO = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
SETTINGS = os.path.join(os.environ["APPDATA"], "pinball_decryptor",
                        "settings.json")
SETTINGS_BAK = SETTINGS + ".shotbak173"
...
session_log.LOG_DIR_OVERRIDE = tempfile.mkdtemp(prefix="pad173shot_")
...
def s_quiesce():
    ...
    if country is not None and power is not None:
        country.set("U.S.A.")
        power.set(panel.POWER_CHOICES[2][0])
        log("picked: %s / %s" % (country.get(), power.get()))
        log("launch env adds: %s" % panel._machine_env())
    info = {"running": "1", "state": "techalerts", "auto": "0",
            "auto_result": "mainslock", "procs": "12", "cpu": "96",
            "rss": "412", "host_cpu": "9", "fps": "57.0"}
    # A poll already in flight when the pollers were stopped still lands, and
    # on this machine it can carry a REAL run's status: apply the synthetic
    # one, then shut the door, and apply it again just before the snap.
    real_apply = panel._apply
    real_apply(info)
    panel._apply = lambda *_a, **_kw: None
    SYNTH.append(lambda: real_apply(info))
    log("state label = %r" % panel._vals["state"].cget("text"))
    log("state tip = %r" % panel._copy_badge.icon_tip.text)
    root.update_idletasks()
    try:
...
def s_snap():
    ...
    root.update_idletasks()
    panel = win._emulate_panel
    log("state label at snap = %r" % panel._vals["state"].cget("text"))
    snap(OUT)
