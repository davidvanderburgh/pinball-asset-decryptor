...
| venom_le | 1.07.0 (2026-09-07) | ✅ live, 107 | ❌ none shipped | not re-measured | ✅ real (topper) — **the `n/a` here was wrong**, seen live 2026-09-07 (item 101); window **800x480**, its own panel record. ⚠️ still LANDSCAPE: the panel is portrait-mounted on the machine, and the rotation is not derivable — its present quad maps u to x with no mirror or transpose, and its attract loop is an abstract creature with no text to read an orientation off. **TURNED from 2026-09-07 (item 102)**: window 480x800, picture rotated, direction settled against a photograph of the owner's own machine after the first attempt came out 180 degrees wrong | ✅ David's green check 2026-09-06 ("venom", build not named) | 2026-09-06 |
| turtles_le | 1.58.1 **or** 1.59.0 — not recorded | ✅ live, 96 | ❌ none shipped | not re-measured | n/a | ✅ David's green check 2026-09-06 ("turtles", build not named) | 2026-09-06 |
| uncanny_xmen_le | 0.97.0 | ✅ live, 110 | ❌ none shipped | not re-measured | n/a | ❌ 2026-09-06: "UPDATING NODE BOARD RUNTIME / UPDATE FAILED 12" = the topper's coil4_lednode claimed the 0x01 guess, the live registry says variant 0x10 (nbdir prior added); playfield a blank black field = 3 shooter-bezel lamps on the cabinet-front image were the whole layout (gate now needs 8 usable devices; this build ships NO playfield drawing, the Schematic is all it can give; **0.98.0 DOES ship one** — `TestMode/xmen_pre_playfield_scaled.png` beside `xmen_pro_...`, found 2026-09-07 (item 101) on the card in `D:\Pinball\incoming`, and since neither is named "le" an LE now takes the PREMIUM drawing by rule rather than by alphabetical luck; **and the WINDOW draws it from 2026-09-07 (item 102)** — it was still blank until then because `layout_art()` refused any picture that did not contain EVERY positioned device, and one of this title's 185 sits past the right edge of the 321-wide drawing. Verified live); ✅ David's green check 2026-09-06 on the restart, "boots clean now" | 2026-09-06 |
...
| metallica_spike | 1.03.0 | ✅ live, 106 | ✅ metallica_playfield…png (item 57 fix) | ✅ 502/664 inside, 0 outside (item 57 fix) | n/a | ✅ David's green check 2026-09-05, nothing reported | 2026-09-05 |
...
| foo_fighters_le | 1.03.0; **1.04.0 reads 105 rows again from 2026-09-07 (item 102)** - it read NOTHING on that build, because swelf.py's stored addresses were measured on the previous one; derived now, no address kept | ✅ static (swelf.py), live-verified | — | — | n/a | ✅ clean, 2026-08-28 (David) | 2026-08-19 |
...
| mando_le | 1.44.0 | ✅ static (swelf.py), live-verified | ❌ none shipped — the card carries 113 PNGs and every one is Insider Connected UI (`images/Connectivity/TestMenu`); no `Test`/`TestMode` folder, no drawing anywhere on the card, and this library holds only this one mando build | ❌ 0 records — the binary carries no device table (names, wire or coordinates), so the window is the schematic | ⚠️ real — the $1999 holographic TOPPER (David-confirmed) — but **measured EMPTY 2026-09-05**: `picture: d2 STILL BLACK after 417 presented frames - the game is composing this display's scene and the scene is empty`, on a run whose glass looped `UPDATING NODE BOARD RUNTIME / UPDATE FAILED / 12`. **Node 12 IS the topper's board** (`hdmi_ws2812node`, "HDMI SPI WS2812", the TOPPER 1..32 RGB LEDs): `node_ident.txt` carried `variant=0x01 variant_guess=1`, and `hexreg.py` off the live process read **0x0c** (class 5, 1.19.0), so the game graded the topper board status 7 and walked the update forever. Fixed the same day (item 67): `nbdir.VARIANT_PRIOR` row, and the shim's registry safety net now RESCANS on a miss — its single scan ran at node 1's claim when the game had decrypted 2 images, 7 exist minutes later, which is also why it was silent on turtles (item 55). **Second run, node 12 clean: STILL EMPTY** (`d2 STILL BLACK after 427 presented frames`, 20 min, no `d2 FIRST`). The topper scene RUNS - video channel 1 decodes 1280x800 clips of scene `ed4ec8f0...` (`Topper_RazorCrestHologram`) and hands the game 30 frames/s - and the game never uploads them (TEXDIRECT stays at the backbox's 30/s, no 1280x800 Map/alloc ever registers, `swap content d2` mask 0). Ruled out: `/dev/mxc_hdmi` (fire-and-forget), `/dev/fb2` (mode set only), the IPU, a second render thread, node 3. The engine plays extra-display clips through `Radium::Video::ExtraVideoPlayer`; its upload path is the open question. The census + a live op dump then showed the topper pass is `clear FBO 1, present texture 2` with NO draw into FBO 1 ever - the scene is composed empty - and the one thing this rig told the game differently was the display size: 1360x768 (eglshim's fall-through) for a 1280x800 panel. The game's own FB_SetTiming records say fb0 1360x768 / fb2 1280x800; `display2.py` now reads them out of the ELF and watch.sh exports PAD_GL2_W/H from them (item 65's size half, card-derived). It was not: the fifth run (size right, node clean) stayed black, and register hooks on the render thread's two per-display draw loops read the gate itself - the display-2 pass calls only `RenderForDisplay(1)` (the base no-op for every class the topper posts) unless renderer byte +0x104 is set, and the game's `topper` class sets it from `node_table[12].part / 100 == 5208530`: **the topper renders only when node 12 reports Stern part 520-8530** in its `f9 00` runtime-info reply (bytes 8..11, which the game formats `%09d` into `ddd-dddd-dd`), and this shim had always answered f9 with zeros - every node read `000-0000-00` live. The game's own board catalog gives `hdmi_ws2812node` exactly that part once its record is framed from the part word (nbdir had credited every type with the next row's number for its whole life). `nbdir.py` now writes `partno=` per node and the shim answers it, and **the topper DRAWS** (`d2 FIRST at frame 277`) - mirrored, because the game presents display 2 through a quad with u reversed (a Pepper's-ghost panel carries the mirror image; padglhost reads that quad and un-mirrors the [display 2] window, `PAD_GL2_MIRROR=0/1` overrides), and cropped, because the game presents it through DISPLAY 0's 1360x768 viewport - which only works when both geometries are equal, so the topper HDMI must run at 1360x768 on the machine - **confirmed the next run: at the backbox's size nothing is cut** - and the 1280x800 `FB_SetTiming` record is the panel's timing request, not GL geometry; watch.sh no longer exports it AS THE RENDER SIZE (`display2.py` stays as the timing reader). **2026-09-07 (item 101): it exports it as the WINDOW size.** The panel record is what the panel IS, so it names the host window and never the geometry the game is told - a separate pair, `PAD_GL2_WIN_W/H`, which `padglhost` letterboxes the 1360x768 render into. Measured with xdotool on a live run: [display 2] is **1280x800** and the topper is still whole and un-mirrored. The un-mirror is read from the buffer the display-2 present draw uses (the first probe answered on another sprite's quad); **verified by David the same evening: the topper draws, complete, text the right way round** | ✅ David's green check 2026-09-05 ("text reads correctly now") | 2026-09-05 |
| rush_le | 1.18.0 | ✅ static (swelf.py), live-verified | ❌ none shipped | ❌ 0 records — the binary carries no device table | n/a | ✅ David's green check 2026-09-05, nothing reported. **2026-09-07 (item 101): it was the SLOWEST title to start and now is not** — 96 s to `running.`, of which 70 s was four separate walks of a 184.6 MB `.data` section (godzilla_le's whole binary is 8.0 MB). 37 s on the first start of a card, 2.3 s on every one after. Reported by peanuts as a 0.187.1 regression; it was not, the shape dates from item 52 | 2026-09-07 |
| star_wars_le | 1.30.0 | ✅ live, 104 | ❌ none found | not re-measured | ✅ real (mini display above the targets, David-confirmed); **window 480x272 since 2026-09-07 (item 101)** — the panel its own FB_SetTiming record asks for, measured live, where it used to open at the backbox's 1360x768 | ✅ David's green check 2026-09-05, nothing reported | 2026-09-05 |
...
| stranger_things_le | 1.12.0 | ✅ static (swelf.py, item 52) | ❌ none found | not re-measured | ✅ real (projector, item 44) - was an EMPTY SCENE until 2026-09-05: the projector's model check reads the cabinet identity record (EEPROM 0..51, see the sweep log) and a BLANK record reads as Pro, which switches the projector off; hwshim seeds a "999" record (the game picks its own build's model) when the block is blank. **Window 848x480 since 2026-09-07 (item 101)**, its own panel record. NOTE the report that asked for this said 368x214, which is the projector CLIP's resolution, not the panel's - a window that size would crop the panel rather than fix the black borders | ✅ David's green check 2026-09-05, projector clip on display 2 | 2026-09-05 |
| sword_of_rage_le | 1.18.0 | ✅ static (swelf.py, ROOTS_NONUM), live-verified, 98 | ❌ none shipped | ❌ no device table | n/a | ❌ SEGV ~40 s (2026-09-06): a printf handed text where a %s pointer belonged (libc strlen under vfprintf, r0 = the bytes "1/1."), not reproduced in 3 rig runs; hwshim now REFUSES such a printf and logs its format + caller (`[printf] REFUSED`, PAD_PRINTF_GUARD=0 off) instead of dying; ✅ David's green check 2026-09-06 on the restart, "stays up now" | 2026-09-06 |
| munsters_le | 1.27.0; **1.28.0 reads 103 rows again from 2026-09-07 (item 102)** - it read NOTHING on that build, because swelf.py's stored addresses were measured on the previous one; derived now, no address kept | ✅ static (swelf.py, ROOTS_NONUM), live-verified, 103 | ❌ none shipped | ❌ no device table | n/a | ✅ David's green check 2026-09-05, nothing reported | 2026-09-05 |
