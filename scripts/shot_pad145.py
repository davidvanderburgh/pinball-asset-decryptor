"""PAD-145: "Only the boot menu" is ticked for a card no SD card holds yet.

    python scripts/shot_pad145.py <out_dir> <before|after>

Two shots, each as ``<before|after>_<screen>.png``:

``never_flashed``
    The flash dialog as the Multi-boot tab opens it for a card that was
    built in an EARLIER run and has never been written onto an SD card - a
    flash ticked on its own in Build / flash card, which is the tester's
    "quick check".  PAD-144 only kept the tick off when the build ran in the
    same Start, so it still came up ticked here.

``already_flashed``
    The same card once a flash of it has finished (the after-code records
    it, the way a real flash's success does).  The tick is on by default
    again - the menu-only write is still one tick away for the card it is
    for.  Before the fix nothing was recorded, so this shot matches the
    first one.
...
device path that does not exist, and no Start is pressed.  The app opens into
Stern with a scratch project (settings.json is backed up and restored) and the
session log is sandboxed.
"""
import ctypes
import json
import os
...
SETTINGS = os.path.join(os.environ["APPDATA"], "pinball_decryptor",
                        "settings.json")
SETTINGS_BAK = SETTINGS + ".pad145bak"
...
scratch = tempfile.mkdtemp(prefix="pad145_")
...
settings.pop("flashed_images", None)        # no flash on record to start with
...
session_log.LOG_DIR_OVERRIDE = tempfile.mkdtemp(prefix="pad145_log_")
...
AFTER_CODE = hasattr(win, "_remember_flashed_image")
log("app records finished flashes: %s" % AFTER_CODE)
...
def snap_dialog(name):
    d = DIALOGS[-1]
    log("%s: menu tick=%s state=%s note=%r" % (
        name, d._menu_var.get(), d._menu_chk.state(),
        d._menu_note.cget("text")))
    d._dlg.update()
    grab(user32.GetAncestor(d._dlg.winfo_id(), 2), name)  # GA_ROOT
...
def hand_off():
    """What Build / flash card's Start calls for a flash ticked on its own:
    the tab's flash_fn with the card already on disk, not fresh."""
    panel = win._multiboot_panel
    root.after(0, lambda: panel._flash_fn(CARD, fresh=False))
...
@step(9000)
def s_open_never_flashed():
    hand_off()


@step(1800)
def s_snap_never_flashed():
    snap_dialog("never_flashed")
    close_dialog()


@step(800)
def s_open_already_flashed():
    if AFTER_CODE:
        win._remember_flashed_image(CARD)
    hand_off()


@step(1800)
def s_snap_already_flashed():
    snap_dialog("already_flashed")
    close_dialog()
