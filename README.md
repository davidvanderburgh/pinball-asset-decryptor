...
9. **Emulate tab** *(Stern Spike 2 — Windows via WSL2, Linux, or macOS in a container)* — run the
   ...
   `libc6-dev`, `e2fsprogs`, `fuse3`, `python3-tk`). Two compilers,
   because two different things get built: the hardware shim is ARM and
   the renderer that draws the picture is a native binary for your own
   PC, so having one of them says nothing about having the other.
