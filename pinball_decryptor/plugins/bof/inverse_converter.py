...
def apply_source_edits(pck_dir, baseline_mtime, log_cb=None, progress_cb=None):
    """Scan ``pck_dir/editable/`` for files newer than ``baseline_mtime``
    and re-encode each one back into its imported binary under
    ``pck_dir/.godot/imported/``.

    Returns ``{"updated": [(src_name, imported_name)], "skipped": [...]}``.
    """
    long_prefix = "\\\\?\\" if sys.platform == "win32" else ""
    ...
    editable_dir = os.path.join(pck_dir, "editable")
    if not os.path.isdir(editable_dir):
        _log("No pck/editable/ folder — nothing to re-encode.", "info")
        return {"updated": [], "skipped": []}

    candidates = []
    for name in os.listdir(long_prefix + os.path.abspath(editable_dir)):
        full = os.path.join(editable_dir, name)
        try:
            mtime = os.path.getmtime(long_prefix + os.path.abspath(full))
        except OSError:
            continue
    ...
    for i, name in enumerate(candidates):
        ...
        if imported is None:
            ...
            continue
        src_full = os.path.join(editable_dir, name)
        try:
