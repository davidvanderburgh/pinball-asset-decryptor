...
| `replace_audio` | True | Replace-Audio tab. BoF audio is imported Godot binaries; Extract writes editable `.wav`/`.ogg` under `pck/_EDITABLE ASSETS/` and `audio_slot_dirs()` restricts the scan to those folders (via the shared `_editable_roots()` walk) |
| `replace_video` | True | Replace-Video tab. Unlike audio, BoF's clips are **standalone `.ogv` PCK entries** at `pck/assets/videos/`, so there is no editable-folder indirection and no `video_slot_dirs` override — see *Other asset types* below |
...
**`may_extractor.extract_pck()`** ([may_extractor.py](../../pinball_decryptor/plugins/bof/may_extractor.py)) dispatches to one of two paths:

- **`_extract_via_directory()` — the authoritative path, and what real builds take.** Calls `pck_directory.read()` and writes every entry from its exact `{ofs, size}`, verifying each slice against the md5 the directory records. Entries are streamed off an `mmap`, so peak RSS stays flat on a 2.9 GB binary containing 80 MB clips. RSCC payloads are decompressed and font magic restored as before. Adds `directory_count` / `md5_ok` / `md5_bad` to the stats dict and sets `via_directory=True`. On Dune this writes **all 5296 entries, all 5296 md5-verified, in ~4.5 s**.
- **The sidecar marker scan — fallback only**, for binaries with no v3 directory (synthetic test stubs, pre-May packs). Scans for `[remap]\n` markers, classifies adjacent vs simple, computes adjacent bounds as `[prev_sidecar_end, this_sidecar_start)` (trimming the `RSCC` separator + zero padding, skipping interleaved `GDSC` blobs), and pairs simple sidecars by magic order.

**Why the directory had to take over (measured on Dune, 5296 entries).** The marker scan can only see files that have a sidecar next to them, and BOF stores plenty that don't. It wrote just **26% of entries byte-correctly**: 53% missing and 21% wrong size. Every one of the **297 standalone `.ogv` videos (1.06 GB) was dropped**, and because their bytes lie between two sidecars each video was folded into the *following* imported texture — a 175 KB poster `.ctex` came out as an 81 MB blob beginning with `OggS`. That over-capture is what made `_decode_ctex`'s `OggS` branch appear to be a real "video-as-texture" format; no `.ctex` in Dune actually holds video. Fonts, `.scn`, `.res` and most `.gdc` were oversized the same way; only `.sample` audio (1025/1029) came out right, which is why the audio workflow worked while everything else quietly didn't. Windows long paths handled with the `\\?\` prefix.
...
<output>/
  ...
  pck/                     (the game's res:// tree, 1:1 with the PCK directory)
    assets/videos/<mode>/… (standalone *.ogv — EDIT THESE IN PLACE)
    assets/…               (lampshows, topper_shows, …)
    .godot/imported/…      (imported binaries — what actually ships)
    .autoconverted/…       (GDRE/script .gdc cache)
    _EDITABLE ASSETS/      (decoded copies of the IMPORTED assets — EDIT THESE)
      audio/  *.wav *.ogg *.qoa
      images/ *.webp *.png
      video/  *.ogv        (empty on Dune — see the video note below)
      fonts/  *.ttf *.otf
      _README.txt
  .bof_modversion          (written by Write, not Extract)
```

Two editing surfaces, and which one an asset uses depends on how BoF stored it: **imported** assets (audio / textures / fonts) are Godot binaries, so they get decoded copies in `_EDITABLE ASSETS/` that `apply_source_edits` re-encodes on Write; **standalone** files (video, and anything else stored plainly) sit at their real `res://` path and are picked up directly by the MD5 walk.
...
- **Video:** BoF ships video as **plain standalone `.ogv` PCK entries**, not as imported binaries — Dune carries **297 of them (1.06 GB, Ogg Theora 1280x720)** under `assets/videos/<mode>/`. They extract to their `res://` path like any other file and need no decoding. Write substitutes them directly: `.ogv` is in `may_packer._SUBSTITUTABLE_EXTS`, and `pck_directory.rewrite()` re-points offsets so a replacement of any size loads. This is the **Replace-Video** surface (`replace_video=True`, `video_slot_exts` pinned to `(".ogv",)`, and deliberately **no** `video_slot_dirs` override — `scan_video_slots`' dot-directory prune already keeps `.godot/imported` out, and the clips are not in the editable folder). Verified end-to-end on the real Dune binary: swapping a 185 KB clip for a 254 KB one repacks in 1.7 s and all 5296 entries still hash to their recorded md5.
  *Legacy note:* `_decode_ctex` still exports a `.ctex` whose payload begins with `OggS` as `.ogv` ([source_converter.py:79](../../pinball_decryptor/plugins/bof/source_converter.py#L79)), with `encode_video_to_ctex` as its passthrough inverse. On Dune that branch only ever fired because of the old extractor's over-capture — no `.ctex` really holds video, and `_EDITABLE ASSETS/video/` now comes out empty. Both are kept in case another title genuinely uses the form. (unverified on Winchester / Labyrinth)
...
- **Script/scene modding is still off.** `.gdc`/`.scn`/`.res` extract byte-exact now, but they aren't in `may_packer._SUBSTITUTABLE_EXTS`, so edits to them are ignored at Write. Audio, textures, fonts and video are the supported surfaces.
- **The extractor's `OggS`-in-`.ctex` branch is a fossil of a bug.** Before extraction was directory-driven it folded each standalone video into the following texture, so 65 Dune "textures" began with `OggS` and were exported to `_EDITABLE ASSETS/video/` as 7–51 MB pseudo-clips (chained Ogg streams plus the real texture trailing on the end). Those files were not game resources at all — they had no PCK directory entry — so replacing one could never have worked. If you are looking at an extract made before this fix, its `_EDITABLE ASSETS/video/` and most of its `images/`, `fonts/`, `.scn`, `.res` and `.gdc` are wrong; re-extract.
- **The PCK directory of ABSOLUTE offsets is load-critical — `may_packer` rewrites it ([pck_directory.py](../../pinball_decryptor/plugins/bof/pck_directory.py)).** BoF's real builds are **stock Godot 4.5.2 v3 packs**. The stock `file_count` at header offset 96 is 0 (why GDRE refuses them and why `may_extractor` marker-scans inline sidecars instead) — but the engine is unmodified `PackedSourcePCK::try_open_pack`, which in v3 reads a **u64 `dir_offset` at header offset 32**, seeks `pck_start+dir_offset`, reads `file_count` cleartext, then reads the real file directory: each entry is `{u32 path_len, path, u64 ofs, u64 size, md5[16], u32 flags}` with data at `pck_start+file_base+ofs` — an **absolute offset**. The running game locates every resource by these offsets, so a naive size-changing repack that doesn't update the directory strands the offsets of all later files → the engine reads *boot* resources at the wrong place → black screen (even editing a gameplay-only asset; the "validated" gate only checks USB-transfer integrity). Two directory flavours, auto-detected from `pack_flags` bit0 `PACK_DIR_ENCRYPTED`: **Dune** (`GBOF` magic) wraps the directory body in `FileAccessEncrypted` (AES-256-CFB128, FAE magic GDFE→EBOF); **Winchester** (`GDPC` magic, flag clear) stores it **plaintext**. `pck_directory.rewrite()` decrypts (if needed), updates every entry's `ofs` by the cumulative byte delta of edits physically before it, sets edited entries' `size`/`md5`, re-serialises + re-encrypts (byte-deterministic CFB), repoints `dir_offset` + the trailer `pck_size`, and streams the result — so a replacement of **any size** repacks correctly and `encode_wav_to_sample` no longer trims. The encrypted directory's AES key is recovered **statically, per-binary** (no runtime dump): `Security::TOKEN` via the engine's key-derivation code fingerprint, then `script_encryption_key` brute-forced over `.data` against the directory's own md5 oracle (`_discover_key`). Derivation: `v=(TOKEN[i]+2*key[i])|key[i]` (unmasked), `out[i]=((v<<4)|(v>>4))&0xff`. Verified end-to-end on **both** real games and by `test_bof_pck_directory.py`. The legacy size-neutral marker-scan walk in `may_packer.pack_pck` is the fallback when no v3 directory is present (synthetic stubs / pre-May). Needs `pycryptodome` (already an installer dep). See memory `reference_bof_pck_alignment`.
...
- [`may_packer.py`](../../pinball_decryptor/plugins/bof/may_packer.py) — native repacker: `pack_pck` dispatches to `_pack_via_directory` (the v3-directory path, any-size edits) when a directory is present, else the legacy size-neutral marker-scan walk (`_read_pck_entries`, RSCC re-wrap, streaming rewrite, GBOF-magic preservation).
- [`pck_directory.py`](../../pinball_decryptor/plugins/bof/pck_directory.py) — the Godot v3 PCK directory of absolute offsets: `read` (encrypted Dune / plaintext Winchester), generic AES-key discovery (`_discover_key`/`_derive_key`/`_find_token`), and `rewrite` (offset-shift + size/md5 update + re-encrypt + repoint `dir_offset`/trailer, streamed). Lets edits of any size repack correctly.
