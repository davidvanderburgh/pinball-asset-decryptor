...
are pointed at it.  The panel half is in test_webui_multiboot.py.
"""
import os
...
def test_a_media_source_on_another_machine_comes_back_as_recorded_and_is_said(tmp_path):
    ...
    form = multiboot_core.MultibootForm(images=rows, out=str(tmp_path / "card.multi.raw"),
                                       sound_move=r0.music, sound_confirm="synth")
    errs = multiboot_core.validate_form(form, sources=False)
    assert errs and all(("not found" in e) for e in errs)
    assert multiboot_core.media_file_errors(errs) == errs
    assert multiboot_core.sound_file_errors(errs) == [e for e in errs if e.startswith("The move")]
    assert multiboot_core.media_file_errors(["Volume is 0-100.", "Image 0: the title must not contain | ; $ or `."]) == []
...
def test_a_media_source_that_is_here_is_kept_as_the_spec_without_a_word(tmp_path):
    ...
    rep["images"][0]["art_source"] = multiboot_core.wsl(str(clip)) + "@3"
    rep["images"][0]["anim_source"] = multiboot_core.wsl(str(clip)) + "@2"
...
def test_parse_extract_reads_the_tools_lines():
    ...
    assert mapping == {0: multiboot_core.host_path("/mnt/d/rec/Orchestral.raw"),
                       1: multiboot_core.host_path("/mnt/d/rec/godzilla_le-1_16_0.raw")}
    assert media == multiboot_core.host_path("/mnt/d/rec/card.multi.menu-media")
...
def test_recover_rows_points_the_rows_at_the_files_and_the_media_copies(tmp_path):
    ...
    assert [multiboot_core.art_spec(r) for r in base] == [multiboot_core.art_spec(r) for r in rows]
    assert multiboot_core.diff_forms(
        multiboot_core.MultibootForm(images=base),
        multiboot_core.MultibootForm(images=rows)) == ([], [])
...
def test_extract_args_name_the_card_the_folder_the_images_and_the_media(monkeypatch):
    ...
    assert args[:2] == [multiboot_core.MKMULTICARD, "extract"]
