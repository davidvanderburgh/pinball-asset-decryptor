...
Foundation milestone: the games DB is auto-generated from PinMAME's
``degames.c``, so these focus on the data-driven detection contract and
the minimal identify+unpack extract — all against synthetic in-memory
zips, so they're CI-safe (no real ROMs needed).
"""
...
def _make_rom_zip(out_path, game_roms, sound_roms):
    """Minimal MAME-style zip carrying the named ROM files (zero payloads)."""
    with zipfile.ZipFile(out_path, "w") as zf:
        for name in list(game_roms) + list(sound_roms):
...
# ---------------------------------------------------------------------------
# Extract pipeline (identify + unpack)
# ---------------------------------------------------------------------------

def test_data_east_extract_e2e(manufacturers_by_key, tmp_path):
    de = manufacturers_by_key["data_east"]
    info = GAME_DB["lethal_weapon_3"]
    z = _make_rom_zip(tmp_path / f"{info['family']}.zip",
                      info["game_roms"], info["sound_roms"])
    out = tmp_path / "out"
    out.mkdir()
    pipe = de.make_extract_pipeline(
        str(z), str(out), None, None, None, None)
    result = run_pipeline_sync(pipe)
    assert result.success, result.summary
    game_dir = out / "lethal_weapon_3"
    assert (game_dir / "game_info.txt").is_file()
    roms = game_dir / "roms"
    assert roms.is_dir()
    unpacked = {p.name for p in roms.iterdir()}
    for name in info["game_roms"] + info["sound_roms"]:
